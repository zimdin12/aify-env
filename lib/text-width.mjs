...
//: EVERYTHING A TERMINAL ACTS ON RATHER THAN DRAWS, as one alternation, so a sequence is removed
//: whole rather than leaving its tail behind as text. In order: a CSI (`ESC [` ... final byte, SGR
//: among them); an OSC, DCS, SOS, PM or APC string up to BEL, ST or the end of the text; any other
//: escape (`ESC c`, `ESC 7`, `ESC =`...); and a bare C0 or C1 control. Written with `\x` escapes so
//: no raw control byte sits in this file.
const TERMINAL_CONTROL = /\x1b\[[0-?]*[ -\/]*[@-~]|\x1b[\]PX^_][\s\S]*?(?:\x07|\x1b\\|$)|\x1b[ -\/]*[0-~]?|[\x00-\x1f\x7f-\x9f]/g;
const SGR_EXACT = new RegExp(`^${SGR_PATTERN}$`);
//: Controls that stand for SPACE on a screen. Kept as a space rather than dropped, so words either
//: side of a newline or a tab do not run together.
const WHITESPACE_CONTROL = /^[\t\n\v\f\r]$/;

/**
 * Text and colour only: what may be written onto somebody else's screen as part of a row.
 *
 * THE FRAME WRITER MODELS ROWS AND COLOUR AND NOTHING ELSE (v0.7 scan, F3 and F4). An alternate-screen
 * switch, a reset, a scroll region or a clipboard write inside a row changes the terminal under a
 * writer that cannot know, and a newline splits one row into two -- so every row after it lands
 * somewhere the writer does not think it is. Colour is the one escape a row may carry, because
 * `width` and `clipToWidth` both account for it.
 *
 * @param {{keepColour?: boolean}} [options] `keepColour: false` drops SGR too, for plain text
 */
export function displayable(text, { keepColour = true } = {}) {
  return String(text ?? "").replace(TERMINAL_CONTROL, (sequence) => {
    if (keepColour && SGR_EXACT.test(sequence)) return sequence;
    return WHITESPACE_CONTROL.test(sequence) ? " " : "";
  });
}
