 ...
 * the whole parent. A pane line is a slice of terminal output and a process title is a slice of a
 * JSON body, so almost every key here arrives that way. MEASURED before this existed: 32 cached
 * slices of 120 units, taken from one-MiB parents, held 40,634,424 bytes — against 3,840 logical
 * units cached. The entry bound and the key-length bound were both being honoured the whole time,
 * which is why neither could catch it. (External review of c74927a.)
 *
 ...
 *
 * BOUNDED FOR REAL NOW: at most WIDTH_MEMO_ENTRIES keys of at most WIDTH_MEMO_MAX_KEY units, which
 * is about 4 MB of owned characters and nothing else.
 */
