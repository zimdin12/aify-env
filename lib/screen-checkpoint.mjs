...
// (DECSTBM), a saved cursor (DECSC), cursor visibility and character sets. Replaying the bytes after a
// snapshot through a screen missing one of those draws a different screen -- measured for margins and
// the line-drawing set. So the two that apps set and then RESET, margins and the active charset, make a
// boundary unclean exactly like a half-parsed sequence: the snapshot waits for them to be put back,
// and gives the replay after the bounded wait. A saved cursor and custom tab stops are NOT declined:
// neither is ever reset, so declining on them would disable checkpoints for the life of the process.
//
// AND A SCREEN SIZE THE DAEMON WILL NOT HOLD. The runner accepts resizes up to 10,000 x 10,000, and an
// emulator that size cost 1.16 GB and a 3.2 s synchronous stall for one snapshot, measured -- every
// agent's output held meanwhile. Past `MAX_CHECKPOINT_CELLS` the checkpoint lets go, for good, and
// subscribers get the replay they had before this file existed.
//
...
//: The largest screen a checkpoint holds: 1000 x 250, far past any real console, a few megabytes.
export const MAX_CHECKPOINT_CELLS = 250_000;
const fits = (cols, rows) => cols * rows <= MAX_CHECKPOINT_CELLS;
...
export function loadCheckpointFactory() {
  loading ??= (async () => {
    ...
    return (geometry) => (fits(geometry.cols, geometry.rows) ? new ScreenCheckpoint({ ...emulator, SerializeAddon }, geometry) : null);
  })();
  return loading;
}
...
class ScreenCheckpoint {
  ...
  #open = new Set();
  #waiting = [];
  //: The last OSC 0/2 title and OSC 9 payload, taken by the emulator as it parses, so they are in
  //: step with the rows. Sanitised as Herdr does (control characters dropped, 256 characters kept).
  #title = "";
  #progress = "";
  ...
  constructor({ Terminal, Unicode11Addon, SerializeAddon }, { cols, rows }) {
    ...
    const core = this.screen.term._core;
    ...
    this.usable = typeof handler?._parser?.currentState === "number"
      ...
      && typeof core?.buffer?.scrollTop === "number"
      && typeof core?.buffer?.scrollBottom === "number"
      && typeof core?._charsetService?.glevel === "number";
    const clean = (text) => [...String(text ?? "")].filter((ch) => !/\p{Cc}/u.test(ch)).slice(0, 256).join("");
    this.screen.term.onTitleChange((text) => { this.#title = clean(text); });
    this.screen.term.parser.registerOscHandler(9, (data) => { this.#progress = clean(data); return false; });
  }

  /**
   * The screen as it stands after everything queued so far: one string per viewport row (trailing
   * blanks trimmed, a wide character's spacer skipped), with the latest title and progress. Null once
   * disposed. Plain text for a reader deciding what the screen SAYS; `snapshot` is for redrawing it.
   *
   * @returns {Promise<{rows: string[], title: string, progress: string}|null>}
   */
  text() {
    if (this.screen.disposed) return Promise.resolve(null);
    return new Promise((resolve) => {
      const waiter = { resolve, timer: null };
      this.#open.add(waiter);
      this.#enqueue("", () => {
        const { buffer, rows } = this.screen.term;
        const active = buffer.active;
        const lines = [];
        for (let row = 0; row < rows; row += 1) lines.push(active.getLine(active.baseY + row)?.translateToString(true) ?? "");
        this.#settle(waiter, { rows: lines, title: this.#title, progress: this.#progress });
      });
    });
  }
  ...
  resize(cols, rows) {
    if (this.screen.disposed) return;
    if (!fits(cols, rows)) {
      this.dispose();
      return;
    }
  ...
  #atBoundary() {
    ...
    return handler._parser.currentState === 0 && handler._stringDecoder._interim === 0
      && scrollTop === 0 && scrollBottom === this.screen.term.rows - 1
      && charsets.charset === undefined && charsets.glevel === 0;
  }
