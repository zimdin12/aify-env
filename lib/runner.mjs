...
export class Runner {
  ...
  #makeEmitter(stream, handleListeners, id = "") {
    ...
    //
    // PER STREAM rather than shared: the carried bytes belong to one byte sequence, and splicing
    // stdout's partial character onto stderr's next chunk would invent a character neither sent.
    // A decoder handed a string returns it unchanged, so the pty path is unaffected.
