...
import { loadCheckpointFactory } from "./screen-checkpoint.mjs";

/**
 * What a process inherits from the Herdr pane this daemon runs in, and must not believe is its own.
 *
 * NO PROCESS STARTED HERE RUNS IN THAT PANE. A daemon started from a Herdr pane passes `HERDR_ENV=1`
 * and `HERDR_PANE_ID` to every child, so each aify launcher among them claimed the DAEMON's pane:
 * renamed it after itself and recorded its own command for Herdr's restore. Measured 2026-09-14 on
 * a dedicated instance: all three claim records named the daemon's terminal, and its pane carried the
 * last worker's label, so a restore would relaunch that worker into the daemon's pane.
 */
const INHERITED_PANE_IDENTITY = ["HERDR_ENV", "HERDR_PANE_ID", "HERDR_TAB_ID", "HERDR_WORKSPACE_ID"];

/**
 * `env` without the pane identity. `undefined` still means "inherit" when there is nothing to remove,
 * so an ordinary daemon's children are started exactly as before. The Herdr socket and binary stay:
 * a worker's own pane is reported through them.
 */
export function withoutPaneIdentity(env, base = process.env) {
  const source = env ?? base;
  if (!INHERITED_PANE_IDENTITY.some((name) => name in source)) return env;
  const cleaned = { ...source };
  for (const name of INHERITED_PANE_IDENTITY) delete cleaned[name];
  return cleaned;
}
...
export class Runner {
  ...
  constructor(deps = {}) {
    ...
    // A SCREEN CHECKPOINT PER PTY (`screen-checkpoint.mjs`). `null` turns it off; a loader that
    // resolves null -- the optional packages are absent -- leaves every stream exactly as before.
    this.#loadCheckpoint = deps.loadCheckpoint === undefined ? loadCheckpointFactory : deps.loadCheckpoint;
  }
  ...
  async start(spec) {
    ...
    // LOADED BEFORE THE SPAWN, so the checkpoint exists before the first byte can arrive. One that
    // missed the start of the output would be a truncated replay with extra steps.
    const makeCheckpoint = useTerminal && this.#loadCheckpoint
      ? await this.#loadCheckpoint().catch(() => null)
      : null;
    ...
    const stream = {
      buffer: "",
      // WHETHER THE REPLAY IS THE WHOLE HISTORY OR A SUFFIX OF IT.
      //
      // `buffer` is capped at `#replayBytes` and silently loses its head, which is fine for a LOG and
      // is not fine for a SCREEN: a consumer replaying a truncated suffix into a terminal emulator
      // cannot recover the positioned text, SGR state or mode changes that fell off the front, so the
      // picture it reconstructs may be confidently wrong. Review traced exactly this and called it
      // out -- the feed emitted a truncated suffix with nothing on the wire to say it was truncated.
      //
      // A FLAG RATHER THAN A BYTE COUNT, because what a consumer can DO with it is binary: either the
      // replay is a complete history it can trust as a baseline, or it is not and the consumer must
      // say so rather than draw a screen that looks authoritative.
      truncated: false,
      // WHETHER THE RETAINED BYTES WERE WRITTEN AT THE SIZE THEY WOULD NOW BE REPLAYED AT.
      //
      // A LATE SUBSCRIBER GETS THE RETAINED TEXT PLUS THE CURRENT GEOMETRY, and if the PTY has been
      // resized since those bytes were written, those two do not belong together. Measured against
      // the real parser: 60 characters and an `O` written at 80 columns, then a resize to 40. The
      // real terminal reflows and shows one row; a subscriber replaying the same bytes into a
      // 40-column emulator puts the `O` on row two. The reconstruction is coherent and wrong, and
      // the baseline rule called it sound.
      //
      // THE SAME SHAPE AS `truncated` AND THE SAME REMEDY. Both mean "these bytes cannot rebuild the
      // positioned screen", and both stop mattering the moment the process throws its screen away
      // and draws it again. So it travels as a flag, fails closed, and is cleared by nothing short
      // of a full repaint on the consumer's side.
      //
      // SET ON ANY RESIZE, not only on one that would visibly change the reflow. Deciding which
      // resizes matter needs the emulator this side does not have, and a rule that guessed would be
      // wrong exactly where the wrapping is subtle.
      //
      // TWO COUNTERS RATHER THAN A LATCH, and that is what keeps the answer USEFUL. A boolean set for
      // ever would refuse the pane for the whole life of any process whose web console had ever been
      // opened -- one resize, a permanent notice, and an agent may go a very long time without
      // emitting the RIS that would clear it. What actually matters is narrower: whether the RETAINED
      // window straddles a resize. Once the resize has scrolled out of the replay buffer, every byte
      // a subscriber receives was written at the size it will be replayed at, and the answer goes
      // back to sound on its own.
      producedTotal: 0,
      resizedAtTotal: -1,
      listeners: new Set(),
      // TOLD WHEN THE PTY CHANGES SIZE. A console reconstructing a screen has to follow, because
      // identical bytes wrap differently at a different width -- so a consumer that learned the
      // geometry once renders every row after a resize in the wrong place, and the two screens
      // diverge silently from that moment. Geometry is not a fact you read once; it is a stream.
      resizeListeners: new Set(),
      //: Resizes the PTY has taken, so a checkpoint can say which of them it already includes.
      resizes: 0,
      checkpoint: null,
    ...
    if (makeCheckpoint && Number(child.cols) > 0 && Number(child.rows) > 0) {
      try {
        stream.checkpoint = makeCheckpoint({ cols: Number(child.cols), rows: Number(child.rows) });
      } catch {
        // No checkpoint is the behaviour this had before it could have one.
      }
    }
    ...
    this.#wireChild({
      ...
      finish: (code, signal) => {
        ...
        stream.exited = true;
        // THE SCREEN GOES WITH THE PROCESS. Nothing will draw on it again, and a subscriber arriving
        // after this gets the replay and the exit, which is the whole of what is left to show.
        stream.checkpoint?.dispose();
  ...
  #spawnChild(spec, useTerminal) {
    if (useTerminal) {
      ...
      return this.#openTerminal(spec.command, spec.args ?? [], {
        cwd: spec.cwd,
        env: withoutPaneIdentity(spec.env),
  ...
  #makeEmitter(stream, handleListeners, id = "") {
    ...
    const deliver = (text) => {
      ...
      if (id) {
        ...
        // WHEN IT LAST SAID ANYTHING, taken at the one place every byte from every process passes
        // through. A view can then say which lanes are producing and which have gone quiet without
        // asking any service what its agents are doing -- which is the boundary this repo keeps.
        this.#registry.markActivity(id, Date.now());
      }
      ...
      const grown = stream.buffer + text;
      // SET WHERE THE LOSS HAPPENS, not inferred later from a full-looking buffer. A buffer that is
      // exactly at the cap is indistinguishable from one that was trimmed, so a consumer guessing
      // from the length would call a complete history truncated on the run where it happened to fit.
      if (grown.length > this.#replayBytes) stream.truncated = true;
      stream.buffer = grown.slice(-this.#replayBytes);
      // COUNTED HERE, where every unit this process produces passes exactly once. It is what turns
      // "was there ever a resize" into "is the resize still inside what a subscriber would be sent".
      stream.producedTotal += text.length;
      stream.checkpoint?.feed(text);
  ...
  /**
   * What a consumer needs to know BEFORE it trusts the replay it is about to receive.
   *
   * A CONSOLE CANNOT DRAW A SCREEN FROM BYTES ALONE, and until now bytes were all this feed sent.
   * Two facts are missing from them and neither can be inferred:
   *
   *   `cols`/`rows` -- THE PRODUCER'S GEOMETRY. Identical cursor-addressing bytes produce different
   *   screens at different widths, and not merely narrower ones: a row-1 overflow WRAPS onto row 2
   *   and collides with what belongs there. A consumer that guesses the width renders a different and
   *   wrong picture, so the width has to come from the side that owns the PTY. Zero for a piped
   *   process, which has no terminal and therefore no size -- a fact, not a missing value.
   *
   *   `truncated` -- WHETHER THE REPLAY IS A COMPLETE HISTORY. The buffer is capped and drops its
   *   head, which is fine for a log and wrong for a screen: the positioned text and SGR state that
   *   fell off the front cannot be recovered, so a reconstructed picture may be confidently wrong. A
   *   consumer told this can say "incomplete" instead of drawing something authoritative-looking.
   *
   * NULL for an unknown id, matching `subscribe`, so "no such process" stays distinguishable from
   * "a process that has produced nothing yet".
   */
  streamMeta(id) {
    const stream = this.#streams.get(id);
    return stream ? this.#meta(id, stream) : null;
  }

  #meta(id, stream) {
    // ASKED OF THE PTY, not of a copy taken at start. `resize()` changes the child's size, and a
    // stored number would be stale from the first resize onward -- which is the same argument the
    // start path already makes for reading `child.cols` rather than echoing what was requested. A
    // process whose child has been released reports 0, which is honest: nothing is running to have a
    // size.
    const child = this.#live.get(id) ?? null;
    return {
      cols: Number(child?.cols) || 0,
      rows: Number(child?.rows) || 0,
      truncated: stream.truncated === true,
      // SO A CONSUMER CAN TELL THAT `cols` AND `rows` ABOVE DO NOT DESCRIBE THE RETAINED BYTES.
      // Without it the meta frame is internally consistent and externally wrong: it reports the
      // geometry honestly and the replay honestly, and says nothing about the fact that they were
      // never true at the same moment.
      //
      // TRUE ONLY WHILE THE RESIZE IS STILL INSIDE THE REPLAY. The oldest retained unit sits at
      // `producedTotal - buffer.length`; a resize older than that was applied before anything a
      // subscriber will receive, so every unit it gets was written at the size it is told about.
      resized: stream.resizedAtTotal >= stream.producedTotal - stream.buffer.length
        && stream.resizedAtTotal >= 0,
   ...
   */
  subscribe(id, listener, onExit, onResize) {
    const stream = this.#streams.get(id);
    return stream ? this.#attach(stream, listener, onExit, onResize, true) : null;
  }

  /** What a process's screen shows as text (`ScreenCheckpoint.text`), or null with no checkpoint. */
  screenText(id) {
    return this.#streams.get(id)?.checkpoint?.text() ?? Promise.resolve(null);
  }

  /**
   * Subscribe as a SCREEN: `onMeta` first, then either the checkpointed screen or today's replay, then
   * live output, resizes and exit -- the order the output route writes them.
   *
   * THE HOLD IS WHAT MAKES IT EXACT. The checkpoint's snapshot resolves later, after the emulator has
   * parsed up to the moment this was called, and output and resizes keep arriving meanwhile. They are
   * held, and when the snapshot lands its `at` and `resizes` say which of them it already includes:
   * the rest are delivered in order, then the subscriber goes live. Nothing is skipped and nothing is
   * delivered twice, and a test drives that with output flowing and compares screens.
   *
   * WITHOUT A CHECKPOINT, or when one cannot be taken, it is `streamMeta` and `subscribe` unchanged.
   *
   * @returns {Function|null} unsubscribe, or null when there is no such process
   */
  subscribeScreen(id, { onMeta, onOutput, onExit, onResize }) {
    const stream = this.#streams.get(id);
    if (!stream) return null;
    const replay = () => {
      onMeta(this.#meta(id, stream));
      return this.#attach(stream, onOutput, onExit, onResize, true);
    };
    if (!stream.checkpoint) return replay();

    const held = [];
    const holdOutput = (text) => held.push({ text, end: stream.producedTotal });
    const holdResize = (size) => held.push({ size, seq: stream.resizes });
    stream.listeners.add(holdOutput);
    stream.resizeListeners.add(holdResize);
    let left = false;
    let live = null;
    stream.checkpoint.snapshot().then((snap) => {
      stream.listeners.delete(holdOutput);
      stream.resizeListeners.delete(holdResize);
      if (left) return;
      if (!snap) {
        live = replay();
        return;
      }
      // THE GEOMETRY THE SCREEN WAS TAKEN AT, which a held resize may since have moved. The replay
      // facts stay honest and `checkpoint` is what makes this sound (`screen-baseline.mjs`).
      onMeta({ ...this.#meta(id, stream), cols: snap.cols, rows: snap.rows, checkpoint: true });
      if (snap.data) onOutput(snap.data);
      for (const event of held) {
        if (event.size && event.seq > snap.resizes) onResize?.(event.size);
        if (!event.size && event.end > snap.at) onOutput(event.text);
      }
      live = this.#attach(stream, onOutput, onExit, onResize, false);
    }).catch(() => {
      // A subscriber that threw is its own failure. It must not become an unhandled rejection in the
      // process that owns every agent's terminal.
    });
    return () => {
      left = true;
      stream.listeners.delete(holdOutput);
  ...
  #attach(stream, listener, onExit, onResize, replay) {
    if (replay && stream.buffer) {
      try {
    ...
    // OPTIONAL, like `onExit`, so every existing caller is untouched. A consumer that does not model
    // a screen has no use for a geometry change.
    if (typeof onResize === "function") stream.resizeListeners.add(onResize);

    return () => {
      stream.listeners.delete(listener);
      if (onExit) stream.exitListeners.delete(onExit);
      if (onResize) stream.resizeListeners.delete(onResize);
    };
  }
   ...
   *
   * @param {string|Buffer} data a STRING is text and is encoded by the writer; a BUFFER is bytes and
   *   is handed over as they are. Which one it should be is the sender's to say, never this
   *   method's -- see `lib/input-encoding.mjs` for the measurement that settled it.
  ...
  resize(id, cols, rows) {
    ...
    // AFTER THE PTY HAS ACTUALLY TAKEN IT, never before: a refused resize must not tell a console the
    // geometry changed, or it reflows a screen the process is still painting at the old width. The
    // same moment is when the retained bytes stop describing the current size -- recorded here for
    // the same reason and with the same ordering.
    const stream = this.#streams.get(id);
    // WHERE IN THE OUTPUT THIS HAPPENED, so a subscriber can be told whether its replay straddles it.
    if (stream) {
      stream.resizedAtTotal = stream.producedTotal;
      stream.resizes += 1;
      stream.checkpoint?.resize(cols, rows);
    }
    for (const listener of stream?.resizeListeners ?? []) {
      try {
        listener({ cols, rows });
      } catch {
        // A console that throws on a resize keeps its stream; its failure is its own.
      }
    }
    return { ok: true };
  }
  ...
  #release(id, reason) {
    ...
    this.#streams.get(id)?.checkpoint?.dispose();
  ...
  release(id) {
    ...
    // AND ITS EXIT LISTENERS ARE TOLD, with no code and no signal because nobody watched it end.
    // `#release` deleted the stream and fired nothing, so a consumer carrying this process -- the
    // aify-comms plugin forgets the terminal and sends the exit marker from its listener -- never
    // learned it was gone, and the service kept a dead worker attached. ONCE: marked exited and
    // cleared, so a close event that still arrives for this child finds nobody left to tell.
    if (!stream || stream.exited) return;
    stream.exited = true;
    for (const onExit of stream.exitListeners) {
      try { onExit(null, ""); } catch { /* a broken listener must not stop the others being told */ }
    }
    stream.exitListeners.clear();
  }
