...
export class Runner {
  ...
  async start(spec) {
    ...
    const entry = this.#registry.add({ service: spec.service, pid: child.pid ?? 0, terminal: useTerminal });
    ...
    const stream = { buffer: "", listeners: new Set(), exitListeners: new Set(), exitCode: null };
    ...
    this.#wireChild({
      child,
      useTerminal,
      emit: this.#makeEmitter(stream, handleListeners),
      finish: (code) => {
        ...
        stream.exitCode = code ?? 0;
        for (const onExit of stream.exitListeners) {
          try {
            onExit(stream.exitCode);
          } catch {
  ...
  #makeEmitter(stream, handleListeners) {
    return (chunk) => {
  ...
  subscribe(id, listener, onExit) {
    ...
    if (typeof onExit === "function") {
      if (stream.exitCode !== null) {
        ...
        queueMicrotask(() => { try { onExit(stream.exitCode); } catch { /* its own failure */ } });
      } else {
