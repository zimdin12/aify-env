...
export class Runner {
   ...
   * called `stop()`, which ends by issuing `taskkill /PID <pid> /T /F` against that very pid. Against
   * a dead process that call can do nothing useful. On Windows it can do harm: pids are recycled
   * quickly on a host spawning agents continuously, and between the liveness probe and the taskkill
   * the number can belong to something else -- whose whole TREE `/T` then takes with it.
   *
