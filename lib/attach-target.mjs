 ...
 */
export function resolveAttachTarget(processes, wanted, { exactId = false } = {}) {
  if (exactId) {
    const matches = (Array.isArray(processes) ? processes : []).filter(p => typeof wanted === 'string' && wanted !== '' && p.id === wanted);
    return matches.length === 1 ? { id: wanted } : { error: `No process with exact id "${wanted}"` };
  }
