 ...
 * one. The separator is a newline, which neither a registry key nor a URL can contain.
 */
 ...
 */
export async function advertiseTo({ targets = [], body = {}, post, env = {} } = {}) {
  const results = [];
  for (const target of targets) {
    ...
    try {
      ...
      const response = await post(url, body, credentialFor(target, env));
