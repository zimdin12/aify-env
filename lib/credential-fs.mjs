...
export async function inspectCredentialFile({
  ...
  // CASE COLLISION, and it is a custody failure rather than a typo. On Windows and on macOS's
  // default volume `Foo.key` and `foo.key` are ONE file, so two services whose references differ
  // only in case would silently share a credential -- and each would read the other's key while
  // every check above passed. MEASURED here: `lstat("foo.key")` succeeds against a file named
  // `Foo.key`, and `realpath.native` returns the TRUE on-disk name, which is one syscall rather
  // than a directory listing on every beat.
  //
  // It also re-confirms containment against the RESOLVED path, so a reparse point that survived the
  // lstat cannot land the read outside the root.
  try {
    // `realpathSync.native` and not the promises API: `fs.promises.realpath` has no `.native`,
    // and only the native form returns the on-disk CASING, which is the whole question here.
    const real = realpathSync.native(target);
    if (path.basename(real) !== path.basename(target)) {
      return {
        state: CREDENTIAL_INSECURE, value: "", identity: "",
        detail: `resolves to ${path.basename(real)}, whose name differs only in case -- two `
          + "services could be sharing one credential file",
      };
    }
    // BOTH SIDES CANONICALISED. Comparing the resolved path against the requested one directly is a
    // false positive on Windows, where a temp path arrives in 8.3 form (`ADMINI~1`) and
    // `realpath.native` returns the long name -- the same directory under two spellings. Measured:
    // it refused every credential the store had just written.
    if (path.dirname(real) !== realpathSync.native(path.dirname(target))) {
      return {
        state: CREDENTIAL_INSECURE, value: "", identity: "",
        detail: "resolves outside the directory it was looked up in",
      };
    }
  } catch (failure) {
    // A path that cannot be canonicalised is not one to read a secret from.
    return {
      state: CREDENTIAL_UNREADABLE, value: "", identity: "",
      detail: `could not resolve the real path: ${failure?.code || failure}`,
    };
  }
...
export async function writeCredentialFile({
  ...
  // A COLLIDING WRITE IS REFUSED BEFORE ANYTHING IS STAGED. The read path detects a requested/on-disk
  // case mismatch, but that is too late for a WRITE: on a case-insensitive volume, storing
  // `foo.key` REPLACES an existing `Foo.key`, so another service loses its credential before any
  // reader is in a position to notice. The damage happens first and the detection second.
  //
  // A directory scan is the only way to see it -- `lstat` succeeds on the wrong case by definition,
  // which is the whole problem. It costs one readdir per WRITE, which is rare, and never per beat.
  const colliding = (await listCredentialFiles(root))
    .find((name) => name !== ref && credentialRefsCollide(name, ref));
  if (colliding) {
    return {
      ok: false, state: CREDENTIAL_INSECURE,
      detail: `${colliding} already exists and differs only in case; storing ${ref} would replace `
...
/**
 * Every credential file present in the store.
 *
 * AN ABSENT STORE AND AN UNREADABLE ONE ARE DIFFERENT ANSWERS. This swallowed both into `[]`, which
 * is the failure-reads-as-absence shape the whole carrier exists to end: an orphan check running
 * over a root it could not read would report a clean store, and a colliding-write guard consulting
 * it would find no collisions because it found nothing at all.
 *
 * A store that has never been created is genuinely empty and says so. Anything else is reported.
 */
...
/** The same listing as a typed answer, for callers that must not throw. */
export async function listCredentialStore(root) {
  try {
    const names = await fsp.readdir(root);
    return { names: names.filter((name) => credentialRefProblem(name) === "").sort(), problem: "" };
  } catch (failure) {
    if (failure && failure.code === "ENOENT") return { names: [], problem: "" };
    return { names: [], problem: `the credential store could not be listed: ${failure?.code || failure}` };
  }
}
