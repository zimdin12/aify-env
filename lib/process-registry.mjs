...
/**
 * Whether this output DRAWS rather than appends.
 *
 * A log is a sequence of lines. A TUI is a picture painted by moving the cursor: `ESC[row;colH` to
 * position, `ESC[K` to erase, `ESC[A`-`G` to step. `lib/pane-buffer.mjs` models `
` and `` and
 * nothing else, which is right for a log and cannot be right for a picture -- replaying a paint into
 * a line model concatenates fragments that were never on the same row.
 *
 * THE SAME PATTERN `cleanTail` ALREADY USES, exported rather than copied. Two regexes for one
 * question is how they start disagreeing, and this repo has paid for that shape repeatedly.
 */
export function drawsWithCursor(text) {
  if (typeof text !== "string" || !text) return false;
  // `.test` on a /g regex advances lastIndex between calls; build a fresh one rather than carry state.
  return new RegExp(CURSOR_MOVE.source).test(text);
}

/**
 * A title a process set, made safe to put on somebody else's screen.
 *
 * THIS IS THE ONLY THING A MANAGED AGENT WRITES DIRECTLY INTO THE OPERATOR'S VIEW. It arrives in an
 * OSC sequence the process chooses, was stored with `.slice(0, 120)` and nothing else, and was
 * rendered into a frame addressed by absolute cursor position. So an agent that printed
 * `ESC]0;ESC[2J...BEL` CLEARED THE OPERATOR'S SCREEN on every poll, and one with a newline in its
 * title split a frame line in two and desynchronised every row below it for the rest of the render.
 *
 * Neither needs malice: a title is often built from a shell prompt or a task description, and both
 * routinely carry tabs and escapes.
 *
 * WHAT SURVIVES: printable text. Control characters, ESC and DEL are dropped rather than replaced,
 * because a title is decoration -- there is nothing here worth showing a placeholder for. The length
 * cap stays, and is applied AFTER stripping so a title padded with control bytes cannot use up the
 * budget.
 */
export function sanitizeTitle(title) {
  return [...String(title ?? "")]
    .filter((ch) => {
      ...
      // C0, DEL and C1. Everything a terminal would act on rather than draw.
...
export class ProcessRegistry {
  ...
  setTitle(id, title) {
    const entry = this.#entries.get(id);
    if (!entry) return;
    entry.title = sanitizeTitle(title);
  }

  /**
   * Record that a process emitted something, and when.
   *
   * PTY ACTIVITY IS THE ONLY STATUS THIS TIER MAY DERIVE, and it is enough. herdr's own rule --
   * "PTY activity is the working authority; screen patterns corroborate" -- is exactly the split
   * this repo already enforces from the other direction: the screen-pattern half was implemented
   * here once, at 5am on 2026-09-03 to unblock a fleet, and the operator ruled it the wrong layer.
   * It now lives in aify-comms as `console_prompts.py`. Bytes are not a service's concept; what a
   * particular runtime's screen MEANS is.
   *
   * So a `blocked` state is deliberately absent from this tier. Reading one here would need to know
   * what a claude confirmation dialog looks like, which is the knowledge that was moved out.
   *
   * The clock is PASSED IN, like every other timestamp here, so a test can drive this to any moment
   * and nothing in this class can flicker.
   */
  markActivity(id, atMs) {
    const entry = this.#entries.get(id);
    // Unknown ids are a no-op for the same reason `setTitle`'s are: output can outlive the entry by
    // a tick, and an activity stamp is never worth throwing over.
    if (!entry) return;
    if (!Number.isFinite(atMs)) return;
    entry.lastOutputAtMs = atMs;
  }
