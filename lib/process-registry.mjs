...
export class ProcessRegistry {
   ...
   *
   * PTY ACTIVITY IS THE ONLY STATUS THE HOST CORE MAY DERIVE, and it is enough. The rule, as the
   * operator amended it on 2026-09-14: the core stays PTY-only and exposes screen text and PTY
   * activity, never what a runtime's screen means; a service's screen knowledge may live in its own
   * plugin directory (`lib/plugins/<service>/`), next to the authoritative screen. The 2026-09-03
   * ruling had moved that knowledge out of the core into aify-comms' `console_prompts.py`.
   *
   * So a `blocked` state is deliberately absent from this registry. Reading one would need to know
   * what a claude confirmation dialog looks like; the aify-comms plugin knows that, in
   * `lib/plugins/aify-comms/screen-rules.mjs`.
   *
