...
export const NOT_STARTABLE_STATUSES = Object.freeze({
  ...
  shell: "a live worker is at its prompt with background shells running",
...
/**
 * What this host knows and is NOT offering, grouped by the reason it refused.
 *
 * WHY A LIST NEEDS ITS OWN NEGATIVE SPACE. The operator, 2026-09-12: "in aify-env start function i
 * see residents for some reason. it should offer only managed options." The rule above has refused
 * residents since this file's first commit -- measured against their live roster the same day, 18
 * offered and every one `managed`, 12 residents refused -- but a list that shows only what it kept
 * cannot be told apart from a list with no rule at all. So the refusals are counted and shown, and
 * "where are my residents" is answered on the screen instead of by reading this file.
 *
 * SCOPED TO THIS MACHINE, like the offer itself. Another host's agents are not missing from a list
 * that says "on this host"; counting them would make every machine look like it was hiding things.
 *
 * GROUPED BY THE REFUSAL'S OWN WORDS, never by a category list beside it. A second vocabulary here
 * would drift from the one that decides, and the drift would be invisible: the counts would still
 * add up.
 *
 * @returns {Array<{count: number, why: string}>} commonest first, then alphabetical
 */
export function notOffered(roster, { machineId = "" } = {}) {
  const table = roster && typeof roster === "object" ? roster.agents : null;
  const mine = lower(machineId);
  if (!table || typeof table !== "object" || !mine) return [];
  const counts = new Map();
  for (const [id, agent] of Object.entries(table)) {
    if (!id || !agent || typeof agent !== "object") continue;
    if (lower(agent.machineId ?? agent.machine_id) !== mine) continue;
    const verdict = startabilityOf(agent, { machineId });
    if (verdict.startable) continue;
