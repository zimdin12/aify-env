...
//
// AND IT CLOSES THE SPACE WHEN THE WORKER GOES. The operator, after the first working spawn: "it
// seems that killing just killed process in that space, but it did not kill the space where agent
// was running." A space outliving its worker is not a neutral leftover -- it is an empty window
// labelled with a running agent's name, and after a few kills the screen describes a fleet that is
// not there. The exit is taken from the RUNNER rather than from the stop call, so a worker that
// crashes or is killed from outside takes its space with it too.
//
 ...
 * @param watchExit how this opener learns the worker is gone, so the space goes with it. Absent
 *   means the space is opened and never closed, which is the behaviour this replaced.
 */
export function paneOpenerFor({
  ...
  return async function openSpaceFor(record) {
    ...
    try {
      ...
      // THE WORKSPACE, NOT THE PANE, is what closes: `workspace create` makes a space holding one
      // pane, so closing the pane alone would leave the empty space the operator reported.
      ...
      if (watchExit && workspaceId) {
        // REGISTERED AFTER THE PANE IS RUNNING, so a worker that exits during the shell poll closes
        // the space it was given rather than one that does not exist yet.
        watchExit(record.id, () => {
          try {
            call(binary, ["workspace", "close", workspaceId], { socket, cwd });
            log(`herdr: closed ${workspaceId}, the space for ${record.label || record.id}`);
          } catch (error) {
            log(`herdr: ${workspaceId} outlived ${record.id} (${error?.message || error})`);
          }
        });
      }
