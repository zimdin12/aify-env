...
//
// AND THE WORKER'S STATUS DOT. The worker's own pane runs `attach`, so Herdr sees no agent in it.
// (The pane identity the worker inherited from the daemon's pane is removed by the Runner, for every
// process.) `prepare` points `AIFY_HERDR_PANE_FILE` at a file of the worker's own; once its pane
// exists, the opener reports the agent on it under `herdr:aify` -- the source the aify-wrapper claim
// uses -- and writes the pane id there, which the launcher's state hook reads.
//
...
/** The source aify-wrapper's pane claim reports under. Herdr refuses a state report from any other. */
const AIFY_AGENT_SOURCE = "herdr:aify";
...
export function paneOpenerFor({
   ...
   */
  const closeSoon = (paneId, name, paneFile = null) => {
    // A FILE THAT WILL NOT GO must not keep the pane: on Windows a launcher hook reading it holds it
    // open (EBUSY), and throwing here skipped the close below.
    try {
      if (paneFile) fs.rmSync(paneFile, { force: true });
    } catch (error) {
      log(`herdr: could not remove ${paneFile} (${error?.message || error})`);
    }
    defer(() => {
  ...
  /** Name this worker's pane in the file its launcher reads. Atomic, so a reader never sees half an id. */
  const announce = (paneId, spec, name) => {
    const paneFile = spec?.env?.AIFY_HERDR_PANE_FILE;
    if (!paneFile) return null;
    try {
      // REPORTED BEFORE THE FILE IS WRITTEN, so the worker's first hook cannot be overwritten by this idle.
      const agent = path.basename(String(spec.launcher || spec.command || "")) || name;
      call(binary, ["pane", "report-agent", paneId, "--source", AIFY_AGENT_SOURCE, "--agent", agent, "--state", "idle"], { socket, cwd });
      fs.mkdirSync(path.dirname(paneFile), { recursive: true });
      fs.writeFileSync(`${paneFile}.tmp`, `${paneId}\n`);
      fs.renameSync(`${paneFile}.tmp`, paneFile);
      return paneFile;
    } catch (error) {
      log(`herdr: ${paneId} shows ${name} with no status (${error?.message || error})`);
      return null;
    }
  };

  async function openSpaceFor(record, spec = null) {
    ...
    try {
      ...
      const paneFile = announce(paneId, spec, name);
      if (watchExit) {
        ...
        const watching = watchExit(record.id, () => closeSoon(paneId, name, paneFile));
        ...
        if (watching === null) closeSoon(paneId, name, paneFile);
      }
  ...
  /**
   * The spec a worker should be started with here: a pane file of its own. Called before the start,
   * because the environment is fixed at spawn and the pane comes after.
   */
  openSpaceFor.prepare = (spec) => {
    const workerEnv = { ...(spec?.env ?? env) };
    workerEnv.AIFY_HERDR_PANE_FILE = path.join(dedicatedRoot, "worker-panes", randomUUID());
    return { ...spec, env: workerEnv };
  };
  return openSpaceFor;
}
