 ...
 *
 * IT WAS REMOVED BY THE OPERATOR'S RULING, not replaced by a guard. 2026-09-12: "it is same env. just
 * herdr is management helper basically" and "it should act same way as outside of herdr". So a
 * dedicated instance reads the host's registry, starts its service plugins, ADVERTISES, HEARTBEATS
 * and CLAIMS under this machine's environment id -- exactly as an ordinary aify-env does, with the
 * same consequence: starting one is starting this host's environment, and supersession reaps
 * whichever aify-env was serving before it. THERE IS NO `claimsHostWork` GUARD. An earlier version of
 * this comment named one; it never existed in the code, and a test that believed an instance was
 * isolated from the service pointed a real daemon at the live one. Found by review, 2026-09-13.
 *
