...
import { width } from "./text-width.mjs";
...
/**
 * Take a screen of the view's own, and give it back (v0.7 scan, F13).
 *
 * THE FIRST FRAME USED TO ERASE THE OPERATOR'S SCREEN rather than switch to an alternate one, and the
 * cursor stayed visible, sweeping across every repainted row. The view now draws on the alternate
 * screen with the cursor hidden, and leaving restores the operator's screen and cursor as they were.
 * The caller owns when: a one-shot render must stay on the screen it was printed to.
 */
//:
//: AND BRACKETED PASTE IS ON WHILE THE VIEW HAS THE SCREEN (v0.7.1 review, T06), so the terminal marks
//: every paste and the view can keep one from acting as keys; leaving turns it off again.
export const ENTER_VIEW = `${CSI}?1049h${CSI}?25l${CSI}?2004h`;
export const LEAVE_VIEW = `${CSI}?2004l${CSI}?25h${CSI}?1049l`;
...
export function frameUpdate(previous, next) {
  ...
  if (first) {
    ...
    // NO ERASE AFTER A ROW ON A SCREEN THAT WAS JUST CLEARED: there is nothing to its right to erase,
    // and a row that fills the terminal leaves the cursor in the pending-wrap state, where an erase
    // takes the glyph in the last column with it (v0.7 scan, F13).
    for (let row = 0; row < after.length; row += 1) {
      out.push(moveToRow(row + 1), after[row]);
    }
  ...
  for (let row = 0; row < after.length; row += 1) {
    ...
    //
    // AND ONLY WHEN THE NEW ROW IS NARROWER than the one on screen (v0.7 scan, F13). A row at least as
    // wide covers every cell the old one used, and a row that fills the terminal leaves the cursor in
    // the pending-wrap state, where an erase takes the glyph in the last column with it -- which is
    // how every full-width heading rule lost its final character.
    out.push(moveToRow(row + 1), after[row]);
    if (width(after[row]) < width(before[row] ?? "")) out.push(ERASE_LINE);
  }
