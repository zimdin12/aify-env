...
/**
 * @returns {{send: (target: {id: string}|null, data: string) => void,
 *   drainedWithin: (ms: number) => Promise<boolean>}} `send` is the view's `onInput`;
 *   `drainedWithin` says whether every agent's queue emptied in time, for quitting (v0.7.1, E3).
 */
export function createClientInput({ endpoint, fetchImpl = fetch } = {}) {
  ...
  return {
    ...
    //: ONE BOUND FOR ALL OF THEM, waited on together: a quit waits once, not once per agent.
    drainedWithin: async (ms) => {
      const results = await Promise.all([...senders.values()].map((sender) => sender.drainedWithin(ms)));
