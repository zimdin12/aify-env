...
export function claimingCheck({ answered = false, plugins = null } = {}) {
  if (!answered || plugins === null) {
    return unanswered("claiming", "no aify-env answered, so what it claims for is unknown");
  }
  if (!Array.isArray(plugins)) {
    // A daemon too old to report plugins. NOT a failure: it may be claiming perfectly well and
    // simply cannot say, and calling that broken would fire on every host mid-upgrade.
    return unanswered("claiming", "this aify-env does not report its plugins; upgrade it to see this");
  }
  if (plugins.length === 0) {
    return failed(
      "claiming",
