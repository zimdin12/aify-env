...
/**
 * Does this registry text parse AND genuinely hold no services?
 *
 * The distinction the check above turns on. `readServices` is deliberately tolerant, so zero from it
 * means either "parsed, nothing registered" or "did not parse at all" -- and those want opposite
 * things said to an operator.
 */
function declaresNoServices(text) {
  let parsed;
...
export function registryCheck(source) {
  ...
  // AN EMPTY REGISTRY IS A CONFIGURATION, NOT CORRUPTION, and saying otherwise sent an operator
  // looking for damage that was not there. A dedicated `herdr-aify` instance is admitted only with
  // an explicitly empty registry -- that is the scoped-service contract in `instance-bootstrap.mjs`
  // -- so the most common way to see this row is the one case where nothing is wrong at all. The
  // "did not parse" inference below is only sound for text that actually declares services.
  if (services.length === 0 && declaresNoServices(source.text)) {
    return passed("registry", "no services are registered here");
  }
...
/** Read-only picker readiness, separate from a daemon's liveness and claim loop. */
export function pickerReadinessCheck(answer) {
  const id = "agent-picker";
  if (!answer?.ok) return unanswered(id, `picker readiness unknown: ${answer?.error || "no environment answered"}`);
  const status = Number(answer.status || 0);
  const body = answer.body;
  if (!Number.isFinite(status)) return unanswered(id, "the environment returned no usable picker HTTP status");
  if (status >= 400) {
    return failed(id, `picker unavailable (HTTP ${status}): ${body?.problem || "request refused"}`,
      "Check the environment's service plugin and its reported error.");
  }
  if (status < 200 || status >= 300 || !Array.isArray(body?.agents) || typeof body?.problem !== "string") {
    return unanswered(id, "the environment returned no usable picker readiness answer");
  }
  if (body.problem) return failed(id, body.problem, "Check the environment's machine identity and service plugin.");
  return passed(id, "the agent picker can list this host's startable agents");
}
 ...
 */
/**
 * `code-current`: is this daemon running the code that is on its disk?
 *
 * THE COMPARISON ALREADY EXISTED AND ONLY A HUMAN COULD MAKE IT. The banner showed what the running
 * daemon LOADED and `aify-env --version` showed what is on DISK, computed the same way, and the
 * operator was left to run a command and compare two hashes by eye. They said what that was worth:
 * they would never do it. So the daemon reports both on `/health` and this row does the comparing.
 *
 * IT NAMES A RESTART AND NOTHING ELSE, because a restart is what a stale process needs and the files
 * are already correct -- reinstalling would change nothing. And it says so carefully: restarting
 * aify-env supersedes the running daemon and REAPS ITS MANAGED WORKERS, so this row is an operator's
 * decision to make with the agents in front of them, not an instruction to follow.
 *
 * BOTH IDENTITIES ARE PRINTED, not just the verdict. Advice with that cost attached has to be
 * arguable, and an operator who suspects the row is wrong needs the two numbers to check it with.
 */
export function codeCurrentCheck({ answered = false, build = null, codeOnDisk = null } = {}) {
  if (!answered) {
    return unanswered("code-current", "no aify-env answered, so what it is running is unknown");
  }
  const running = String(build || "").trim();
  const onDisk = String(codeOnDisk || "").trim();
  // A DAEMON TOO OLD TO REPORT THE PAIR IS UNANSWERED, NOT CURRENT. This is the failure this
  // project has shipped twice -- a check that gathered no evidence reporting a pass -- and it would
  // land here in the exact case the row exists for: an operator upgrading from a build that predates
  // the field, asking whether their restart took.
  if (!running || !onDisk) {
    return unanswered(
      "code-current",
      "this aify-env does not report both what it loaded and what is on disk, so whether it is running current code is unknown — restart it on a build that does",
    );
  }
  if (running === onDisk) {
    return passed("code-current", `running the code on disk (build ${running})`);
  }
  return failed(
    "code-current",
    `running build ${running}, but the code on disk is ${onDisk} — this daemon loaded its files `
    + "before they changed, so a fix on disk is not a fix that is running",
    "restart aify-env to pick it up — but this reaps the managed workers it is running, so do it when losing them is acceptable, or leave the row red until it is",
  );
}
