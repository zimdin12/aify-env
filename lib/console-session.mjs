...
import { SHUTDOWN, initialFocus, reconcileFocus, routeKey, splitKeys } from "./keys.mjs";
...
export class ConsoleSession {
   ...
   *
   * OUTSIDE THE PANE A READ IS SPLIT INTO KEYS, because a held arrow or a quick `jj` arrives as one
   * read on a busy daemon and used to do nothing. The pane, the picker and a confirmation take the
   * chunk WHOLE, as they always have: the pane passes it to the process untouched, the picker reads
   * it as text, and a confirmation is answered only by a chunk that is exactly `y`.
   *
   * WHEN THE KEYS OF ONE READ CHANGE THE MODE, the rest follows the new mode's rule. Into the picker,
   * the rest is the search. Into the pane or a confirmation, the rest is DROPPED: no frame of the
   * pane has been drawn yet, so it is not a place to type, and a paste must never answer a prompt it
   * opened.
   *
  ...
  handleChunk(data) {
    ...
    while (at < keys.length) {
      ...
      if (!KEY_AT_A_TIME_MODES.has(mode)) {
        // The whole chunk if nothing has been handled yet; the picker's search if keys led into it.
        if (at === 0 || mode === "picker") results.push(this.handleInput(keys.slice(at).join("")));
        break;
      }
      results.push(this.handleInput(keys[at]));
      at += 1;
    }
    return results;
  }
