...
export class PaneBuffer {
  constructor({ maxLines = DEFAULT_MAX_LINES } = {}) {
    ...
    //: When output last MOVED THE CURSOR rather than appending. Null until it ever does.
    this.cursorAddressedAtMs = null;
  }
  ...
  append(chunk, { nowMs = Date.now() } = {}) {
    ...
    if (drawsWithCursor(chunk)) this.cursorAddressedAtMs = nowMs;
   ...
   */
  view({ height = 10, width = 80, nowMs = Date.now() } = {}) {
    ...
    if (this.isPainting(nowMs)) return paintingNotice(width).slice(0, rows);
    const all = this.carry.text ? [...this.lines, this.carry.text] : this.lines;
    return all.slice(-rows).map((line) => clipToWidth(line, width));
  }

  /** Whether this process has painted recently enough that its lines are not readable as lines. */
  isPainting(nowMs = Date.now()) {
    const at = this.cursorAddressedAtMs;
    if (!Number.isFinite(at) || !Number.isFinite(nowMs)) return false;
    // A stamp from the future is not evidence; clocks move and a latched pane is the costly direction.
    if (at > nowMs) return false;
    return nowMs - at <= CURSOR_PAINT_WINDOW_MS;
  }
