...
export class PaneBuffer {
  ...
  view({ height = 10, width = 80, agent = "", screen = null } = {}) {
    ...
    if (this.isPainting()) {
      ...
      // The unsound case was answered above, so a screen reaching here is one to draw.
      if (screen) return screenLines(screen.rows, { width, height: rows });
      return paintingNotice(width, agent).slice(0, rows);
    }
    ...
    if (this.concealSeen) return paintingNotice(width, agent).slice(0, rows);
