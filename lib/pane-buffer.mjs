...
import { displayable } from "./text-width.mjs";
...
export class PaneBuffer {
  ...
  view({ height = 10, width = 80, agent = "", screen = null } = {}) {
    ...
    // A PAINTING STREAM THAT `refusalReason` LET THROUGH HAS A SOUND SCREEN, so it is drawn.
    // Whether a stream is painting is asked of what is RETAINED -- see `isPainting` -- so a process
    // that painted a banner and then logged is a log again once the banner has scrolled out.
    if (this.isPainting()) {
    ...
    // A LOG. A stream that has concealed never reaches this line: `refusalReason` refuses it above,
    // and that refusal closed the third disclosure in this feature. Measured 2026-09-08 against the
    // real parser: a complete, untruncated history of `ESC[8m` + CR + `SYNTHETIC_HIDDEN` renders as
    // four blank rows on a real terminal and printed the token on this path, while every gate written
    // for the first two disclosures passed it.
    //
    // AND ONLY TEXT AND COLOUR LEAVE THIS PATH (v0.7 scan, F3). "No cursor move" is what makes a
    // stream a log; it does not make every other escape in it safe. An alternate-screen switch, a
    // reset, a scroll region or a clipboard write went straight onto the operator's terminal, under a
    // frame writer that models none of them. `displayable` keeps what `clipToWidth` understands.
    const all = this.carry.text ? [...this.lines, this.carry.text] : this.lines;
    return all.slice(-rows).map((line) => clipToWidth(displayable(line), width));
  }
