  ...
  try {
    if (action === "start") {
      return await startTerminal({
        ...
        buildSpec, resolveCandidates, handles, prompts, baseEnv,
      });
    }
    if (action === "input") {
...
async function startTerminal({
  ...
  // ONE WORKER PER AGENT, ENFORCED HERE AND NOT ONLY UPSTREAM.
  //
  // Measured twice on 2026-09-03: two `sc-lead`s, two `sc-coder`s. The service asked for them and
  // was right to -- it believed the first worker was dead, and creating a replacement for a dead
  // worker is exactly what it should do. What was missing is that THIS host already held a live
  // process for that agent and started another anyway.
  //
  // REPLACE, NOT REFUSE. A restart is a legitimate reason to ask again, and refusing would block
  // recovery for the case the service is usually right about. Stopping the predecessor first makes
  // "start a terminal for this agent" mean what it says, and keeps two claude instances out of one
  // working directory -- which is its own failure, independent of the count.
  for (const stale of handles.otherTerminalsFor(launch.agentId, terminalId)) {
    const staleHandle = handles.handleFor(stale);
    log(
      `replacing the worker this host was already running for "${launch.agentId}" `
      + `(terminal ${stale}, process ${staleHandle}) -- one agent gets one worker`,
    );
    try {
      if (staleHandle) await processes.stop(staleHandle);
    } catch (error) {
      // Said, not thrown: failing to stop the old one is not a reason to refuse the new one, and
      // leaving the caller with neither is the worse outcome.
      log(`could not stop the previous worker for "${launch.agentId}": ${error?.message || error}`);
    }
    handles.forget(stale);
    prompts?.forget?.(stale);
  }
  ...
  const carried = processes.subscribe(
    handle,
    (chunk) => {
      // ANSWERED BEFORE FORWARDED, so a worker stuck at its first-run acknowledgment is unstuck in
      // the same tick rather than after a round trip. `prompts` answers exactly one dialog and only
      // once per terminal; everything else on screen is left alone.
      prompts?.observe?.(terminalId, chunk);
...
export async function runTerminalControlPass({
  ...
  await touchLiveTerminals({ api, handles, log });
 ...
 */
async function touchLiveTerminals({ api, handles, log }) {
  for (const terminalId of handles?.terminalIds?.() ?? []) {
    try {
      ...
      await api.terminalOutput(terminalId, { output: "" });
    } catch (error) {
