...
/**
 * The error in a runner verdict that REFUSED, or "" when nothing refused.
 *
 * `Runner.write` and `Runner.resize` answer `{ok:false, error}` for an unknown handle, a process
 * with no input channel, a piped process with no terminal and a non-positive winsize. This handler
 * discarded that and reported `completed` for all of them -- so a keystroke the runner rejected was
 * reported as delivered, and a refused resize became a geometry the service stored as accepted.
 *
 * AN EXPLICIT REFUSAL IS WHAT FAILS. A collaborator returning nothing has made no claim about the
 * write, and treating silence as failure would refuse every substitute that does not answer.
 */
function refusalFrom(result) {
  if (!result || typeof result !== "object") return "";
  if (result.ok !== false) return "";
  return String(result.error || "the runner refused it and gave no reason");
}
...
// THE ONLY IMPORT IN THIS FILE, and the exception is narrow enough to state. Everything else
// arrives by injection, which is what lets these tests drive a whole control pass with no
// network and no filesystem. `output-sender.mjs` is a PURE factory -- it takes its `post` and
// `log` as arguments and touches nothing itself -- so importing it replaces no seam a test needs.
// It is here rather than passed through because a DEFAULT has to name something, and a caller
// that forgot to supply a sender would otherwise get a ReferenceError at the first pty chunk.
import { createOutputSender } from "./output-sender.mjs";
// Pure too: evaluation is handed `read` and `report`, and holds nothing but its own timers.
import { observeScreen } from "./screen-observer.mjs";
 ...
 *
 * AND IT DID DRIFT, found by external review 2026-09-04 (Round 8 M5). This read
 * `["stopped", "failed", "exited", "killed"]` against a service that says
 * `{stopped, failed, lost, ended, completed, cancelled}` -- so a terminal the service had declared
 * `lost`, `ended`, `completed` or `cancelled` read as LIVE here and was never recognised, while
 * `exited` and `killed` were names the service never sends. A comment claiming a mirror is not one,
 * which is this project's own lesson about prose that nothing checks.
 *
 * Latent when it was found -- no current writer produces those four -- and fixed anyway: the whole
 * value of a mirror is that it holds before the day something starts using a name.
 *
 * `mcp/stdio/tests/the-host-mirrors-the-service-terminal-vocabulary.test.js` in aify-comms now reads
 * BOTH lists and fails if they disagree, because a mirror nobody measures is a comment.
 */
export const TERMINAL_ENDED = Object.freeze([
...
/**
 * The environment a launch runs with: this host's own, minus what the service says a worker must never
 * inherit, with the service's overlay on top.
 *
 * WHY THE REMOVAL. An overlay can set a name but never unset one, and some names do harm by being
 * PRESENT: `CLAUDE_CODE_CHILD_SESSION` marks a process as a child session and turns transcript saving
 * off. This host merged `{...process.env, ...launch.env}` and named neither it nor the role alias, so a
 * host started inside a Claude Code session launched every managed claude with its transcript off --
 * the strip had lived in the environment bridge v0.6.2 deleted. The service names the list
 * (`launch.unsetEnv`); a second copy here would agree with it until one of them changed.
 *
 * IN ANY CASE, because Windows reads `claude_code_child_session` as the same variable.
 */
export function launchEnv(baseEnv = {}, launch = {}) {
  const unset = new Set((Array.isArray(launch?.unsetEnv) ? launch.unsetEnv : [])
    .filter((name) => typeof name === "string" && name)
    .map((name) => name.toUpperCase()));
  const kept = Object.fromEntries(Object.entries(baseEnv || {}).filter(([name]) => !unset.has(name.toUpperCase())));
  return { ...kept, ...(launch?.env && typeof launch.env === "object" ? launch.env : {}) };
}
...
export async function runOneControl({
  ...
  // THE ORDERED OUTPUT SENDER, threaded through rather than made here: one per HOST, so a
  // terminal's chunks stay in order across control passes (Round 8 M3). Defaulted because this
  // function is called directly by its own tests.
  sender = createOutputSender({
    post: (id, body) => api.terminalOutput(id, body), log, status: TERMINAL_ATTACHED,
  }),
  withinRoots,
  buildSpec,
  resolveCandidates,
  ...
  try {
    if (action === "start") {
      return await startTerminal({
        ...
        buildSpec, resolveCandidates, handles, baseEnv, sender,
      });
    }
    if (action === "input") {
      ...
      const written = processes.write(requireHandle(), String(control.body ?? ""));
      const writeRefusal = refusalFrom(written);
      if (writeRefusal) {
        await report(api, controlId, CONTROL_FAILED, writeRefusal);
        return { outcome: "refused", controlId, detail: writeRefusal };
      }
      await report(api, controlId, CONTROL_COMPLETED, "", TERMINAL_ATTACHED);
      return { outcome: "completed", controlId };
    }
    if (action === "resize") {
      const resized = processes.resize(
        requireHandle(), Number(control.cols) || 0, Number(control.rows) || 0);
      const resizeRefusal = refusalFrom(resized);
      if (resizeRefusal) {
        // THE SERVICE RECORDS THE REQUESTED GEOMETRY ON A COMPLETED RESIZE, so reporting one
        // here for a refusal stores a width nothing accepted -- and `Runner.resize`'s own
        // comment says why that is worse than an error: the console reflows a screen the
        // process is still painting at the old size.
        await report(api, controlId, CONTROL_FAILED, resizeRefusal);
        return { outcome: "refused", controlId, detail: resizeRefusal };
      }
      await report(api, controlId, CONTROL_COMPLETED, "", TERMINAL_ATTACHED);
      return { outcome: "completed", controlId };
    }
...
async function startTerminal({
  ...
  // Threaded from the pass so every terminal on this host shares ONE ordered sender.
  sender,
}) {
  ...
  // written to close.
  const env = launchEnv(baseEnv, launch);
  let built = null;
  const rejected = [];
  ...
  if (live.length) {
    ...
    if (standing === "ended" || standing === "gone") {
      ...
      const adopted = carryTerminal({ processes, api, handles, log, terminalId, handle, sender, runtime: launch.runtime });
      if (!adopted) {
  ...
  const carried = carryTerminal({ processes, api, handles, log, terminalId, handle, sender, runtime: launch.runtime });
 ...
 */
function carryTerminal({ processes, api, handles, log, terminalId, handle, sender, runtime }) {
  // THE SENDER IS INJECTED, like everything else here, and it is what makes output ORDERED.
  // Until 2026-09-04 each chunk fired its own POST; the service assigns `output_seq` in ARRIVAL
  // order and its own comment says concurrent POSTs scramble the console (Round 8 M3).
  let observer = null;
  const release = processes.subscribe(
    handle,
    (chunk) => {
      observer?.noteOutput(chunk);
      // NOTED BEFORE IT IS SENT, and deliberately: this is the veto that stops the end-status
      // rule from killing a working agent, so it must not depend on the POST succeeding. The
      // process spoke -- that is the fact -- whether the service was reachable to hear it.
      handles.noteOutput(terminalId);
      // Returns immediately: a pty listener may not be made to wait, so the sender queues and
      // coalesces. Failures are reported inside it -- there is nobody above a listener to hand
      // an error to.
      sender.send(terminalId, chunk);
    },
    (code, signal) => {
      ...
      // AFTER THE OUTPUT IT FOLLOWS. This marker carries a status and an exit code, which the
      // ordered sender's `send` cannot express, so it goes out through `api.terminalOutput`
      // directly -- and posted the instant the process ended it OVERTOOK whatever the sender still
      // held. The console then showed `[terminal exited]` and after it the last thing the worker
      // said, which is precisely the screen an operator opens a dead console to read. Reported by
      // review 2026-09-08 as an ordering defect; the tail itself was never lost.
      //
      // BOUNDED, and a service that has stopped answering does not hold the marker for ever -- the
      // wait says whether it actually drained, and a marker sent anyway says so in the log rather
      // than presenting an order it does not have.
      //
      // `sender.forget` MOVED IN HERE WITH IT. Dropping the queue before the wait would make the
      // wait meaningless, and dropping it after means the tail is released once it has gone rather
      // than while it is still going.
      // SAYING SOMETHING MUST NOT BE ABLE TO STOP DOING IT, and the first version of this could.
      //
      // `log` is the CALLER'S function. The warning below sat BEFORE the release and the marker
      // POST, so a logger that throws aborted both -- no release, no exit marker -- and the `.catch`
      // then called the SAME logger and turned it into an unhandled rejection in the daemon. Review
      // drove it with a logger that starts failing after the listener attaches: FIRST, FINAL-TAIL,
      // and no marker at all.
      //
      // This is the same defect `output-sender.mjs` already carries a fix for, written in a
      // different file a few hours later. A diagnostic is contained where it is CALLED, and a
      // wrapper that only guards the happy path is not containment.
      const announce = (message) => {
        try {
          log(message);
        } catch {
          // Deliberately silent: the only thing left to report it with is the thing that failed.
        }
      };
      const marker = (async () => {
        const settled = await sender.drained(terminalId);
        if (!settled) {
          announce(`terminal ${terminalId}: the exit marker is being sent before the queued output `
            + "finished draining, so the console may show it out of order");
        }
        sender.forget(terminalId);
        await api.terminalOutput(terminalId, body);
      })().catch((error) => announce(`terminal ${terminalId} exit not delivered: `
        + `${error?.message || error}`));
      // NOTED so a stopping host waits for it rather than exiting over it.
      handles.noteExitMarker?.(marker);
    },
  );
  // A TRANSITION IS A LIVENESS FRAME CARRYING THE OBSERVATION, sent the moment it is seen rather
  // than at the next pass. No status, like every liveness frame; the answer is the next pass's job.
  observer = release && observeScreen({
    runtime, log, read: () => processes.screenText?.(handle) ?? null,
    report: (activity) => api.terminalOutput(terminalId, { output: "", activity }),
  });
  if (observer) { handles.observedBy?.(terminalId, observer); observer.noteOutput(""); }
  return release;
}
...
/**
 * Tell the service what happened to one control, and NEVER throw doing it.
 *
 * R9-H2, external review 2026-09-05. This is the same defect as Round 8 H2 in the claim loop,
 * surviving in the loop beside it. `api.reportControl` throws on any non-2xx -- a 409 when the
 * operator cancels between acting and reporting, a 404 on a terminal the service has since
 * dropped, a transport blip during a restart. Of the fifteen call sites here, exactly ONE had a
 * `.catch`. From any of the other fourteen the throw escaped `runOneControl`, escaped the pass,
 * escaped `controlForever` -- which had no catch -- and landed in a one-shot `.catch` that logged
 * "terminal control loop stopped". The host then ran no further terminal for the life of the
 * process, invisibly: the heartbeat is a separate loop, so `bridgeLastSeen` stayed fresh and every
 * instrument read healthy.
 *
 * GUARDED HERE, NOT AT THE CALLERS. Fourteen hand-written `.catch`es is a guarantee that lasts
 * until the fifteenth call site is added without one -- which is how this file got here. The
 * reasoning is `claim.mjs`'s `reportOrLog`, word for word: a service that is down or refusing us
 * is not an error this host can act on, and the control has already been carried out either way.
 *
 * Returns whether the report landed, so a caller that wants to know can ask.
 */
async function report(api, controlId, status, error = "", terminalStatus = "", extra = {}) {
  const patch = { status, ...extra };
  if (terminalStatus) patch.terminalStatus = terminalStatus;
  if (error) patch.error = error;
  try {
    await api.reportControl(controlId, patch);
    return true;
  } catch (failure) {
    reportFailureLog(`could not report control ${controlId} as ${status}: ${failure?.message || failure}`);
    return false;
  }
}

/**
 * Where a failed report is written. Module-scope because `report` is a module-level helper with no
 * access to a pass's `log`, and a failure nobody can see is the half of R9-H2 that would remain.
 */
let reportFailureLog = () => {};

/** Point the failure log at this pass's logger. Called by `runTerminalControlPass`. */
export function setControlReportLog(log) {
  reportFailureLog = typeof log === "function" ? log : (() => {});
}
...
export async function runTerminalControlPass({
  ...
  // ONE SENDER FOR THE HOST, passed in by the plugin: ordering must hold ACROSS passes, and a
  // sender created per pass would order within one and interleave between them. Defaulted so
  // this function stays callable on its own, which its tests do.
  sender = createOutputSender({
    post: (id, body) => api.terminalOutput(id, body), log, status: TERMINAL_ATTACHED,
  }),
}) {
  // A FAILED REPORT IS NOW LOGGED RATHER THAN THROWN, and this is where it learns where to write.
  // Without it R9-H2 would be half fixed: the loop would survive and nobody would know why a
  // control never settled.
  setControlReportLog(log);
  let claimed;
  try {
...
async function touchLiveTerminals({ api, processes, handles, log, now = Date.now, quietMs = ORPHAN_QUIET_MS }) {
  // ONLY WHAT THIS HOST STILL RUNS. A frame for a terminal whose process is gone is the ghost console
  // `AN EXITED TERMINAL IS NOT REPORTED ALIVE` exists to prevent, reached through a process that left
  // the runner without an exit listener firing.
  for (const terminalId of heldTerminalIds(handles, processes)) {
    try {
      ...
      const activity = handles.activityFor?.(terminalId);
      const answer = await api.terminalOutput(terminalId, activity ? { output: "", activity } : { output: "" });
