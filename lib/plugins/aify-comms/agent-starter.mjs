...
import { notOffered, restartTargetFor, startabilityOf, startableAgents } from "../../startable-agents.mjs";
...
export class AgentStarter {
  ...
  async list() {
    ...
    if (!this.#machineId) {
      return { agents: [], skipped: [], problem: "this environment cannot say which machine it is, so it cannot scope the list" };
    }
    let roster;
    try {
      roster = await this.#api.agents();
    } catch (error) {
      return { agents: [], skipped: [], problem: `aify-comms did not answer: ${error?.message || error}` };
    }
    // THE REFUSALS TRAVEL WITH THE OFFER. A list showing only what it kept is indistinguishable
    // from a list with no rule, which is exactly what the operator reported -- see `notOffered`.
    return {
      agents: startableAgents(roster, { machineId: this.#machineId }),
      skipped: notOffered(roster, { machineId: this.#machineId }),
      problem: "",
    };
  }
  ...
  async start(agentId) {
    ...
    try {
      // THE PRECONDITION TRAVELS WITH THE REQUEST. `restartTargetFor` chose this session because
      // the listing showed no live session, and that reading is one round trip old by now. A
      // worker starting in between would turn this start into a STOP of somebody's live
      // terminal -- review traced exactly that. Re-reading here a third time would only shorten
      // the window; the authority is the one place that can ask and act on one set of rows.
      const answer = await this.#api.controlSession(target.sessionId, "restart",
                                                   { onlyIfNoLiveSession: true });
