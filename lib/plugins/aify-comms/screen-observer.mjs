...
// once through `report`. `idle`, `working` and `blocked` are the only states it reports; a rule that
...
const REPORTED = new Set(["idle", "working", "blocked"]);
...
export function observeScreen({
  ...
  async function evaluate() {
    ...
    if (!seen) {
      ...
      settle(evaluateScreen(manifest, { screen: screenText(seen.rows), title: seen.title, progress: seen.progress }));
    }
    if (again) {
      again = false;
      wake(throttleMs);
