...
export function observeScreen({
  ...
  function settle(detection) {
    ...
    // `shell` IS AN IDLE VERDICT WEARING ANOTHER NAME (background-shell.mjs refines one), so it has
    // to serve the same wait. Keying on the literal `idle` let a working agent be reported as
    // `shell` from a single mid-turn repaint, which is the case this debounce exists for -- the
    // rename walked straight past it (external review 2026-09-21, finding F).
    const settledFromIdle = detection.state === "idle" || detection.state === "shell";
    const plainIdle = current?.state === "working" && settledFromIdle
