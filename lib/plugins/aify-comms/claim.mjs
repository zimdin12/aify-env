...
export function workspaceWithinRoots(workspace, roots = [], { windows = false } = {}) {
  const normalise = (value) => {
    ...
    const text = slashed.replace(/\/+$/, "") || (slashed.startsWith("/") ? "/" : "");
...
export async function runClaimPass({
  api, environmentId, machineId = "", cwdRoots = [], windows = false, log = () => {},
  // Injected so a test can assert the pid that reaches the service. The daemon passes none.
  pid = process.pid,
}) {
  let claimed;
  try {
    // THE MACHINE ID TRAVELS WITH THE CLAIM, and leaving it out cost an agent its identity.
    // `api.claim` has always accepted one and this call has never passed one, so every claim since
    // aify-env took claiming over on 2026-09-03 carried an empty `claim_machine_id`. aify-comms
    // writes that value onto the agent row when the spawn reaches `running`, so a blank erased the
    // machine the agent belongs to; the worker's own registration normally put it back seconds
    // later, which is why only agents whose spawns FAILED were left blank. One of them then
    // vanished from this environment's start menu, which offers only agents on this machine.
    claimed = await api.claim({ environmentId, machineId });
  } catch (error) {
