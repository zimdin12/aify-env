...
export async function runClaimPass({
  ...
  // THE SAME SPEC BUILDER THE HTTP ENDPOINT USES. A launcher is not a command: its contents decide
  // how it runs, because on Windows nothing reads a shebang and a bash launcher spawned by path
  // simply does not start. Building the spec here by hand is what made this path fail its first
  // end-to-end run with "starting, failed" and no reason -- the Runner refuses a spec with no
  // `fileText`, and no unit test with a fake runner could see it.
  ...
  if (built.error) {
    // THE HOST'S OWN REASON. "cannot read", "refused by the allowlist" and "no POSIX shell to
    // interpret it" send an operator to three different places, and collapsing them into "failed to
    // start" is what this replaces.
    await api.report(request.id, { status: CLAIM_FAILED, error: built.error.detail });
    log(`refused spawn ${request.id}: ${built.error.detail}`);
    return { outcome: "refused", spawnRequestId: request.id, detail: built.error.detail };
  }

  let started;
  try {
    started = await processes.start(built.spec);
  } catch (error) {
