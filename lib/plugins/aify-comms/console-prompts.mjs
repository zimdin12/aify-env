...
// WHAT A TUI ACTUALLY SENDS, and the reason the first version of this could never fire.
//
// MEASURED against a real worker 2026-09-03: claude does NOT send spaces. It moves the cursor. The
// bytes on the wire for the accepting option are
//
//   I<ESC>[1Cam<ESC>[1Cusing<ESC>[1Cthis<ESC>[1Cfor<ESC>[1Clocal<ESC>[1Cdevelopment
//
// so a matcher looking for "I am using this for local development" is looking for a string that is
// never transmitted. The rule sat there for 100 seconds beside a dialog it was written to answer.
// The aify-comms bridge's rules worked because they ran against a RENDERED screen; this stream is
// raw, and the difference is invisible until you print the bytes.
//
// SO THE STREAM IS NORMALISED FIRST, and deliberately not by rendering it: a full terminal emulator
// here would be a second screen model to keep in step with the service's, for one dialog. What is
// needed is only enough to make text on one line read as text on one line.
/** `ESC [ n C` — move the cursor right. THIS IS HOW CLAUDE WRITES A SPACE. */
const CURSOR_FORWARD = /\[(\d*)C/g;
/** `ESC [ row ; col H` — go to a position. In a redrawn dialog, each one starts a new line. */
const CURSOR_POSITION = /\[\d*(?:;\d*)?[Hf]/g;
/** `ESC ] … BEL` — operating-system commands, e.g. the window title. Carries no layout. */
const OSC = /\][^]*(?:|\\)/g;
/** Everything else in the CSI family: colours, modes, erases. */
const CSI = /\[[0-?]*[ -/]*[@-~]/g;
/** Two-character escapes. */
const OTHER_ESCAPE = /[@-Z\\-_]/g;

/**
 * Enough of a terminal to read one line of text.
 *
 * ORDER MATTERS, and both orderings were wrong in the first version for the same reason: stripping
 * a control code that was carrying LAYOUT. Cursor-forward becomes SPACES before anything else eats
 * it, or the words fuse into "Iamusingthis". Cursor-positioning becomes a NEWLINE before the general
 * CSI rule removes it, or every line of the dialog joins into one and "the cursor is on the
 * accepting line" stops meaning anything at all.
 *
 * DELIBERATELY NOT A TERMINAL EMULATOR. A real screen model here would be a second one to keep in
 * step with the service's, for one dialog. This is only enough to make text on one line read as
 * text on one line.
 */
export function renderableText(raw) {
  return String(raw ?? "")
    .replace(CURSOR_FORWARD, (_, n) => " ".repeat(Math.min(Number(n || 1) || 1, 200)))
...
export function answerForConsole(tail) {
  ...
  if (!raw) return "";
  // NORMALISED FIRST. Slicing the RAW tail would cut through an escape sequence and leave a
  // half-code that renders as text; slicing after is safe because what is left is text.
