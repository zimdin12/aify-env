...
export class CommsApi {
  ...
  async #send(method, path, body, timeoutMs = REQUEST_TIMEOUT_MS, requireJson = false) {
    ...
    if (response.status === 204 && !requireJson) return {};
    return response.json().catch(() => {
      if (requireJson) throw new CommsApiError(`${method} ${path} returned no valid JSON`,
        { status: response.status, path });
      return {};
    });
  }
   ...
   */
  //: `heldTerminals`, when given as a list, is every terminal this host holds a live process for.
  //: The service ends this environment's other confirmed terminals on it, so it travels ONLY as a
  //: list: a caller that cannot say sends nothing, and the service reads absent as "an older host,
  //: do nothing" -- never as "holds nothing".
  async heartbeat(advertisement = {}, { heldTerminals } = {}) {
    ...
    return this.#send("POST", "/environments/heartbeat", {
      ...
      metadata: {
        ...
        ...bridgeMetadata,
        ...(Array.isArray(heldTerminals) ? { heldTerminals: heldTerminals.map(String) } : {}),
      },
    });
  }
   ...
   */
  async agents() {
    // Validate before the picker filters the table into an array. A valid empty
    // table is evidence; missing data or a refusal must not become that same [].
    const roster = await this.#send("GET", "/agents", undefined, REQUEST_TIMEOUT_MS, true);
    if (roster?.ok === false) {
      throw new CommsApiError(`agent roster refused: ${roster.error || "no reason given"}`, { path: "/agents" });
    }
    if (!roster || typeof roster !== "object" || Array.isArray(roster)
        || !roster.agents || typeof roster.agents !== "object" || Array.isArray(roster.agents)) {
      throw new CommsApiError("aify-comms returned no usable agent roster", { path: "/agents" });
    }
    return roster;
  }
