...
import { createPromptAnswerer } from "./console-prompts.mjs";
...
export function createCommsPlugin({
  ...
  //: THE ONE PROMPT A FRESH CLAUDE WORKER CANNOT PASS ALONE. `claude-aify` launches with
  //: `--dangerously-load-development-channels`, and claude answers that with a first-run
  //: acknowledgment it then waits at -- the worker registers `online` and never claims
  //: anything. The aify-comms bridge answered it and this host did not, which is why a
  //: worker started here was up but deaf. Built once per plugin so it remembers which
  //: terminals it has already answered: repeated Enter has been measured to do nothing.
  const prompts = createPromptAnswerer({
    // TRANSLATED THROUGH THE HANDLE BOOK. The answerer sees a TERMINAL id, because that is what
    // the console stream is keyed by; the runner answers only to the id it returned from
    // `start`. Handing it the terminal id is the same mistake that left every worker's console
    // empty, and here it would silently write to nothing while reporting the prompt answered.
    write: (terminalId, data) => {
      const handle = handles.handleFor(terminalId);
      if (!handle) throw new Error(`no process for terminal "${terminalId}" on this host`);
      return host?.processes?.write?.(handle, data);
    },
