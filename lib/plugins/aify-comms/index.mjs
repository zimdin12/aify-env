...
export function createCommsPlugin({
  ...
  async function beat() {
    ...
    try {
      ...
      state.claimer = claimer
        ...
        : null;
      // THE REFUSAL IS ACTED ON, not only reported, since 2026-09-04 (external review, Round 8 M4).
      //
      ...
      // and `idle` takes the SHORT floor. Every one of those asks costs the service a
      // `BEGIN IMMEDIATE`.
      //
      // STATE, NOT AN EVENT, deliberately. The service queues a stop for a superseded bridge in
      // `environment_controls`, and nothing drains it -- the consumer was the aify-comms
      // environment-control loop, which v0.6.2 deleted. This flag arrives on every beat instead and
      // cannot be lost.
      const wasRefused = state.claimRefused === true;
      state.claimRefused = Boolean(claimer && claimer.accepted === false);
      if (state.claimRefused) {
        state.lastHeartbeatError = `not the claimer: ${claimer.reason || "refused"}`;
        // ONCE PER TRANSITION. A line every 30 seconds is how a log stops being read, and this one
        // has to still be legible on the beat where it starts.
        if (!wasRefused) {
          host?.log?.(
        ...
        host?.log?.("aify-comms accepted this host as the claimer again; resuming normal claiming.");
      }
    } catch (error) {
  ...
  async function claimForever() {
    ...
    try {
      while (!stopped) {
        ...
        const refused = state.claimRefused === true;
        const pause = (result.outcome === "unreachable" || refused)
  ...
  async function controlForever() {
    ...
    try {
      while (!stopped) {
        // THE LOOP SURVIVES A THROWING PASS, since 2026-09-06 (external review, Round 9 H2).
        //
        // This is the claim loop's Round 8 H2 defect, still standing in the loop beside it: `try {
        // while ... } finally {}` with no catch, and a caller that is a one-shot `.catch(log)`. One
        // throw anywhere in a pass ended terminal control for the life of the process, and did it
        // invisibly -- the heartbeat is a SEPARATE loop, so `bridgeLastSeen` kept refreshing and
        // every instrument read healthy while no terminal could be started, stopped or typed into.
        //
        // The known throw is fixed at its source in `terminal-controls.mjs`, where `report` now
        // logs instead of throwing. This is the CLASS: a pass that fails for a reason nobody
        // anticipated costs one interval, never the loop. `lastControl` records the failure so the
        // instrument moves rather than freezing on its last good value.
        let result;
        try {
          result = await runTerminalControlPass({
            api,
          ...
          host?.log?.(`aify-comms terminal control pass failed: ${error?.message || error}`);
          result = { outcome: "unreachable" };
        }
  ...
  return {
    ...
    capabilities: {
      agents: {
        ...
        start: async (agentId) => (starter
          ? starter.start(agentId)
          : { started: false, agentId: String(agentId || ""), sessionId: "", problem: notStarted }),
      },
    },
    ...
    async start(pluginHost) {
      ...
      starter = new AgentStarter({ api, machineId });
