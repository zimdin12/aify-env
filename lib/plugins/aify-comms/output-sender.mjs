...
export function createOutputSender({ post, log = () => {}, status = "" }) {
  ...
  async function drain(terminalId) {
    ...
    try {
      ...
      // 2026-09-08: removing EITHER alone leaves all seven tests green, and removing BOTH fails
      // three. `state.pending` is cleared BEFORE the await, so the loop's own re-check already sees
      // whatever arrived during it, and no async boundary exists between the loop exiting and this
      // line for anything to arrive in.
      //
      // KEPT ANYWAY, and the reason is the one case the loop cannot cover: an exception escaping the
      // `while` leaves `pending` unsent, and the only throw inside it today is `log()` -- a caller's
      // function, which this module has no business assuming about. So this is a net under a path
      // nothing currently takes, stated rather than implied, because a reader who finds two
      // mechanisms for one job should know which is the primary and that the other is not dead by
      // accident.
