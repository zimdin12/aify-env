...
import { PLAIN, RESET, sameStyle, sgrBetween, styleFrom } from "./screen-style.mjs";
...
export class ScreenEmulator {
  ...
  constructor({ Terminal, Unicode11Addon }, { cols, rows }) {
    ...
    //: WHETHER THE PROCESS HAS ITS CURSOR SHOWN (DECTCEM, `ESC[?25h`/`ESC[?25l`), which the headless
    //: package does not expose. Read off the parser as the sequences are applied, so it is in order
    //: with the screen. Returning false lets xterm's own handling run as well.
    this.cursorVisible = true;
    const noteCursor = (visible) => (params) => {
      if (params.some((p) => p === 25 || (Array.isArray(p) && p.includes(25)))) this.cursorVisible = visible;
      return false;
    };
    this.term.parser?.registerCsiHandler?.({ prefix: "?", final: "h" }, noteCursor(true));
    this.term.parser?.registerCsiHandler?.({ prefix: "?", final: "l" }, noteCursor(false));
    //: A full reset (`ESC c`) shows the cursor again.
    this.term.parser?.registerEscHandler?.({ final: "c" }, () => { this.cursorVisible = true; return false; });
  ...
  /** Whether the process turned bracketed paste on, so a paste reaches it marked, as a terminal would. */
  get bracketedPaste() {
    return this.term?.modes?.bracketedPasteMode === true;
  }
   ...
   */
  rows({ color = false, cursor = false } = {}) {
    if (this.disposed) return [];
    const buffer = this.term.buffer.active;
    const out = [];
    // THE CARET, drawn as the cell under it in inverse (v0.7 scan, F11). The pane is rebuilt from
    // cells, so without this an attached operator typed into a screen with no visible cursor. Only
    // when asked (an attached pane), only with colour (inverse IS an SGR), and only while the process
    // has its cursor shown: a runtime that hides it and paints its own would get a stray block.
    const caretAt = cursor && color && this.cursorVisible
      ? { x: buffer.cursorX, y: buffer.baseY + buffer.cursorY - buffer.viewportY }
      : null;
    for (let y = 0; y < this.term.rows; y += 1) {
      const line = buffer.getLine(buffer.viewportY + y);
      if (!line) { out.push(""); continue; }
      let text = "";
      // ONE STYLE PER RUN, not per cell. A 132-column row with an SGR before every character is
      // kilobytes of escapes for 132 visible glyphs, redrawn every refresh, per pane -- so this
      // emits only what CHANGED, which is what the process itself did on the way in.
      let style = PLAIN;
      for (let x = 0; x < this.term.cols; x += 1) {
        const cell = line.getCell(x);
        if (!cell) continue;
        if (color) {
          // NO SAMENESS CHECK HERE. One was written and NO MUTANT COULD KILL IT: `sgrBetween` already
          // returns "" for an unchanged style, so the guard only skipped concatenating an empty
          // string. Two rules where one will do, and the redundant one is the kind that drifts --
          // "emit on change" now lives in exactly one place and is tested there.
          let next = styleFrom(cell);
          if (caretAt && caretAt.x === x && caretAt.y === y) next = { ...next, inverse: !next.inverse };
          text += sgrBetween(style, next);
          style = next;
        }
        ...
        if (isConcealed(cell)) {
          // AS MANY SPACES AS IT OCCUPIES COLUMNS, not one. A concealed WIDE glyph is two columns and
          // its continuation cell is skipped above, so emitting a single space pulled everything after
          // it one column left -- measured by review: a `B` at column 3 came out at column 2. Removing
          // the content is right; removing its FOOTPRINT is a different screen.
          text += " ".repeat(Math.max(1, cell.getWidth()));
          continue;
        }
        text += cell.getChars() || " ";
      }
      // CLOSED AT THE END OF EVERY ROW THAT OPENED ANYTHING. `sideBySide` puts another pane's text
      // immediately after this one on the same physical line, so a style left open would colour the
      // dashboard beside it -- and on the last row, the operator's shell prompt afterwards.
      if (color && !sameStyle(style, PLAIN)) text += RESET;
      out.push(text);
    }
    return out;
  }
