...
function processStartedAt(pid, { platform, run, readFile = readFileSync, statOf = null }) {
  try {
    if (platform === "linux") {
      // The DIRECTORY's creation, which is the process's: /proc/<pid> appears when it does.
      const stat = statOf ? statOf(`/proc/${pid}`) : statSync(`/proc/${pid}`);
      const at = Number(stat?.ctimeMs ?? stat?.birthtimeMs ?? 0);
      return at > 0 ? at : null;
    }
    if (platform === "win32") {
      const res = run(
