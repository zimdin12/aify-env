...
export function routeKey(data, state) {
  ...
  if (now.mode === "picker") {
    ...
    // AN ESCAPE SEQUENCE IS DROPPED WHOLE, not filtered byte by byte -- and this file's own test
    // caught the difference. Filtering on "printable" removes the ESC and keeps `[` and `C`, so a
    // right arrow types a literal `[C` into the filter, matches nothing, and reads as a broken
    // picker. Every unhandled sequence starts with ESC, so the chunk is refused at its first byte.
    if (chunk.startsWith(ESC)) return stay();
    // PRINTABLE ONLY otherwise, because one chunk may be a paste of several characters.
    const typed = [...chunk].filter((ch) => ch >= " " && ch !== BACKSPACE).join("");
  ...
  if (chunk >= "1" && chunk <= "9") return jumpTo(now, Number(chunk) - 1);
  if (chunk === PICKER_KEY) {
