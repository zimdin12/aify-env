...
//
// AND IN THE DAEMON'S VIEW Ctrl+C IS A QUESTION FIRST (v0.7, F1). One keystroke used to end every
// managed worker on the host, from the menu, the start list and the picker too -- exactly the modes
// where an operator reaches for Ctrl+C to back out. Those modes now close on it, the way `confirm`
// already cancels on it, and a focus carrying `confirmInterrupt` turns the dashboard's Ctrl+C into
// "stop the environment?", which only a single `y` answers. A signal-delivered SIGINT never reaches
// this router, so it keeps its meaning.

/**
 * Every mode this router can put the focus in.
 *
 * DECLARED, BECAUSE RECONCILING USED A HAND-TYPED LIST AND DROPPED THE TWO NEWEST. `reconcileFocus`
 * preserved `pty` and `picker` and reset everything else to `dashboard` -- so `menu` and `confirm`
 * were discarded on EVERY redraw, which is every two seconds and after every keystroke. The actions
 * menu vanished before it could be used and a confirmation prompt was dropped before it could be
 * answered. Shipped and found by review within the hour.
 *
 * It is the same defect `paneHidden` was guarded against one file over: a rebuild that names fields
 * by hand loses the one nobody remembered. A list is not the cure -- this list is -- but it is a list
 * with ONE reader and a test that walks every entry, so a mode that is not in it cannot be routed to
 * in the first place.
 */
export const MODES = Object.freeze(["dashboard", "pty", "picker", "menu", "confirm", "start"]);

/** 0x1d. Leaves `pty` mode, and closes the picker. The one way back, from anywhere. */
export const DETACH = String.fromCharCode(29);

/** What the daemon's Ctrl+C asks about: the whole environment, not one process. */
...
/**
 * The full action vocabulary, in the order a menu shows them.
 *
 * WHAT A MENU ACTUALLY OFFERS IS A SUBSET, chosen by the caller, and that is honesty rather than
 * courtesy. `aify-env` can stop a process it owns; it cannot RESTART a managed agent, because
 * respawning one is the service's business and this tier has no primitive for it. A menu listing
 * `restart` anyway would be a control that does nothing -- the same defect this codebase keeps
 * finding from the other end, where a field has no reader.
 *
 * So the caller declares what it can perform and the view offers that. An entrypoint that wires no
 * executor offers only `attach`, which it can always do.
 */
export const MENU_ACTIONS = Object.freeze(["attach", "restart", "stop"]);

/** The actions this focus offers, defaulting to attach alone -- the one nothing has to wire. */
export function menuActionsIn(state) {
  const declared = Array.isArray(state?.menuActions) ? state.menuActions : null;
  const usable = (declared ?? []).filter((name) => MENU_ACTIONS.includes(name));
  return usable.length ? usable : ["attach"];
}
...
/**
 * The key that opens the list of agents this host could START. `s` for start.
 *
 * A LIST OF ITS OWN, not a row in the actions menu, because it acts on something the dashboard is
 * not showing. Every other mode here narrows or acts on the PROCESS list; these are agents with no
 * process at all -- which is exactly why an operator wants them, and exactly why they cannot be
 * reached from a menu opened on a row.
 *
 * IT DOES NOT CONFIRM, and that follows the rule the confirmation already derives rather than being
 * an exception to it: `DESTRUCTIVE` means "ends work the operator cannot get back". Starting an
 * agent that has no worker ends nothing, and it is undone by the stop that sits one menu away. It
 * still takes three deliberate keystrokes -- `s`, a cursor move, Enter -- which is the same distance
 * the actions menu puts between an operator and a choice.
 */
const START_KEY = "s";

/**
 * Every key the dashboard binds to a NAMED FEATURE, and what to call it.
 *
 * WHY A DECLARATION EXISTS AT ALL. Three places tell an operator what the view accepts -- the hint
 * line, `aify-env --help`, and the README table -- and adding a binding means remembering all three.
 * Nobody does. Measured 2026-09-08 while adding `s`: `--help` was ALREADY missing `p` and `m`, both
 * shipped weeks earlier, so a third of the bindings existed only for whoever had read the source.
 * This file's own header says it: bindings that are not written down are bindings nobody has.
 *
 * THE RECORD IS NOT THE MECHANISM -- the test that walks it is. `the-keys-are-documented.test.js`
 * drives `routeKey` over every printable character and requires that anything producing a named
 * action appears HERE and in all three documents. So a key added with a router branch and no entry
 * fails by BEHAVIOUR rather than by a source regex, which is the difference between proving a line
 * was written and proving the binding is reachable and described.
 *
 * MOVEMENT AND LEAVING ARE DELIBERATELY ABSENT. Arrows, `j`/`k`, the digits and `q` are not features
 * with names; they are how any list works, and the hint line already says so in its own words.
 */
export const VIEW_KEYS = Object.freeze({
  [PICKER_KEY]: "find",
  [MENU_KEY]: "actions for the selected agent",
  [PANE_KEY]: "show or hide the console pane",
  [START_KEY]: "start an agent that has no worker",
  //: THE RECOVERY KEY, declared so the documentation sweep asks for it (v0.7 scan, F15). It was in
  //: no document, and it is the only way back from a smeared screen short of restarting the view.
  [REPAINT]: "redraw the screen",
});

/**
 * How the documents write a key: a control character as `Ctrl+<letter>`, anything else as itself.
 *
 * A control key cannot be searched for as its raw byte in `--help` or the README, which is why the
 * documentation sweep names keys through this rather than using them verbatim.
 */
export function keyName(key) {
  const code = String(key ?? "").charCodeAt(0);
  if (String(key).length === 1 && code >= 1 && code <= 26) return `Ctrl+${String.fromCharCode(64 + code)}`;
  return String(key ?? "");
}
...
export function reconcileFocus(state, count) {
  ...
  if (n === 0) {
    // A MENU AND A CONFIRMATION BOTH ACT ON A PROCESS, so an empty list closes them exactly as it
    // closes an attached pane. Only the picker survives nothing-to-show: a search that currently
    // matches nothing is a legitimate thing to be looking at while a host starts work.
    return {
      // THE START LIST SURVIVES AN EMPTY PROCESS LIST, and this is the case it exists FOR. Its rows
      // are agents with no process, so a host running nothing is exactly when an operator opens it.
      // Closing it here -- as the menu and the pane are closed, for the opposite and correct reason
      // that they act on a row -- would make the feature unreachable on an idle host, which is the
      // host most likely to want it.
      //: AND SO DOES THE QUESTION ABOUT STOPPING THE ENVIRONMENT, because it is about no row. An
      //: idle host is where it is most likely asked, and closing it on the repaint that follows the
      //: keystroke would leave the operator nothing to answer.
      mode: state?.mode === "picker" || state?.mode === "start" ? state.mode
        : asksToShutDown(state) ? "confirm" : "dashboard",
      selected: -1,
      count: 0,
      menuAt: 0,
      confirming: asksToShutDown(state) ? SHUTDOWN : null,
      confirmInterrupt: state?.confirmInterrupt === true,
      startAt: Number.isFinite(state?.startAt) ? state.startAt : 0,
      startCount: Number.isFinite(state?.startCount) ? state.startCount : 0,
      // THE THIRD FIELD THIS RETURN HAS DROPPED. `paneHidden`, then `confirming`, now the offer list
      // -- each rebuilt as a literal and each forgotten once. A session starts at count 0, so this
      // path runs before anything else and silently reset the menu to attach-only.
      menuActions: Array.isArray(state?.menuActions) ? state.menuActions : null,
      query: state?.query ?? "",
  ...
  // ANY DECLARED MODE SURVIVES. Naming the survivors by hand is what lost `menu` and `confirm`.
  const mode = MODES.includes(state?.mode) ? state.mode : "dashboard";
  ...
  // THE MODE'S OWN STATE COMES WITH IT. A surviving `confirm` whose `confirming` was dropped is a
  // prompt that no longer knows what it is asking about, which is worse than one that closed.
  return {
    mode,
    selected,
    count: n,
    query: state?.query ?? "",
    paneHidden,
    menuAt: Number.isFinite(state?.menuAt) ? state.menuAt : 0,
    confirming: state?.confirming ?? null,
    //: Whether this view asks before its Ctrl+C stops everything. Carried like every other field.
    confirmInterrupt: state?.confirmInterrupt === true,
    //: THE START LIST'S CURSOR AND ITS LENGTH, carried like everything else this rebuild names by
    //: hand. This return has dropped a field three times -- `paneHidden`, `confirming`, then
    //: `menuActions` -- and each time the symptom was a control that reset itself within two
    //: seconds. A dropped `startCount` would make Enter in the start list do nothing at all.
    startAt: Number.isFinite(state?.startAt) ? state.startAt : 0,
    startCount: Number.isFinite(state?.startCount) ? state.startCount : 0,
    //: Carried like every other field this rebuild names by hand. A menu that lost its offer
    //: list on the next redraw would fall back to `attach` alone mid-choice.
    menuActions: Array.isArray(state?.menuActions) ? state.menuActions : null,
  };
}

/** Whether this focus is asking to stop the whole environment. */
function asksToShutDown(state) {
  return state?.mode === "confirm" && state?.confirming === SHUTDOWN;
}
...
export function routeKey(data, state) {
  ...
  if (now.mode === "picker") {
    // Ctrl+C BACKS OUT, like Ctrl+]. It used to stop the whole environment from here.
    if (chunk === DETACH || chunk === CTRL_C) {
  ...
  // ── start mode ────────────────────────────────────────────────────────────────────────────────
  //
  // A LIST OF AGENTS, NOT OF PROCESSES, and that is why it carries its own cursor and its own count.
  // `selected` and `count` describe what the dashboard is drawing; moving them here would slide the
  // process selection -- and therefore the pane -- underneath a list that has nothing to do with it.
  //
  // ARROWS AND `j`/`k` BOTH, like the menu and unlike the picker. There is nothing to type into
  // here, so a letter is free to be a cursor key; in the picker it would be a character of the query.
  if (now.mode === "start") {
    // Ctrl+C BACKS OUT, like Ctrl+]. It used to stop the whole environment from here.
    if (chunk === DETACH || chunk === CTRL_C) {
      return { state: { ...now, mode: "dashboard", startAt: 0 }, toPty: null, action: "start-close" };
    }
    if (chunk === REPAINT) return { state: now, toPty: null, action: "repaint" };
    if (chunk === UP || chunk === "k") return moveStart(now, -1);
    if (chunk === DOWN || chunk === "j") return moveStart(now, +1);
    if (chunk === "\r" || chunk === "\n") {
      // NOTHING TO START IS NOT AN ERROR. An empty list means every agent this host knows already has
      // a worker, which is the healthy case -- and reporting a choice for row 0 of an empty list is
      // how a caller ends up starting `undefined`.
      if (!(startCountIn(now) > 0)) return stay();
      return { state: { ...now, mode: "dashboard", startAt: 0 }, toPty: null, action: "chose:start" };
    }
    // EVERY OTHER KEY IS IGNORED rather than falling through to the dashboard, for the reason the
    // menu does it: a `q` or a digit would act on the list behind the one being read.
    return stay();
  }
  ...
  if (now.mode === "menu") {
    ...
    if (chunk === "\r" || chunk === "\n") {
      const offered = menuActionsIn(now);
      const chosen = offered[now.menuAt] ?? offered[0];
      ...
      // ATTACH IS PERFORMED HERE, not handed out. It is the one action this router can carry out
      // itself -- it moves the keyboard and touches no process -- and leaving it to the caller made
      // the menu's own default INERT: `m`, Enter, then typing did nothing, because every executor
      // handles `stop` and none of them knows what attaching means. An offer that does nothing is
      // worse than an offer that is absent.
      //
      // THE SAME TRANSITION Enter makes from the dashboard, including revealing the pane: typing into
      // one nobody can see is not a feature, whichever key got you there.
      if (chosen === "attach") {
        return {
          state: { ...now, mode: "pty", menuAt: 0, paneHidden: false },
          toPty: null,
          action: "attach",
        };
      }
  ...
  //
  // WHERE IT WOULD STOP EVERYTHING, IT ASKS FIRST. The caller that owns the environment says so on
  // the focus, and the answer goes through `confirm`, where only a single `y` says yes.
  if (chunk === CTRL_C) {
    if (now.confirmInterrupt === true) {
      return {
        state: { ...now, mode: "confirm", confirming: SHUTDOWN },
        toPty: null,
        action: `confirm:${SHUTDOWN}`,
      };
    }
    return { state: now, toPty: null, action: "interrupt" };
  }
  ...
  if (chunk === START_KEY) {
    // OPENS EVEN WITH NOTHING TO SHOW, unlike the actions menu. An empty actions menu would offer to
    // stop a process that does not exist; an empty start list is a real answer -- every agent this
    // host knows already has a worker -- and the operator asked the question. Refusing to open would
    // read as the key not working, which is the one thing the screen must never do.
    return { state: { ...now, mode: "start", startAt: 0 }, toPty: null, action: "start-open" };
  }
  if (chunk === "\r" || chunk === "\n") {
...
//: One key at the front of a chunk: a CSI sequence (`ESC [` ... final byte), an SS3 one (`ESC O x`),
//: an escape with one following character (Alt+key), a bare escape, or one code point.
const LEADING_KEY = /^(?:\x1b\[[0-?]*[ -\/]*[@-~]|\x1bO.|\x1b[\s\S]?|[\s\S])/u;

/**
 * Whether a key only moves a cursor. A read made only of these is split and each key handled; any
 * other read is taken whole, so a paste on the list stays inert (v0.7 TUI review: `ok` plus Enter,
 * split, moved the selection and attached the keyboard to another agent).
 */
export function isNavigationKey(key) {
  return key === UP || key === DOWN || key === "j" || key === "k";
}

/**
 * A chunk as the keys it holds, in order (v0.7 scan, F14).
 *
 * A READ IS NOT A KEY, and outside the pane that used to mean a held arrow or a quick `jj` that
 * arrived in one read did nothing at all, because routing compared the whole chunk against one key.
 * Escape sequences stay whole, and everything else is split by CODE POINT so a character outside the
...
export function splitKeys(chunk) {
  ...
  while (rest) {
    const [key] = rest.match(LEADING_KEY);
    keys.push(key);
...
/**
 * How many agents the start list currently holds, defaulting to none.
 *
 * NONE IS THE SAFE DEFAULT. A state that has never heard of this field -- every literal written
 * before it existed, and a caller building one by hand -- must not report a choice for a row nobody
 * supplied. Enter on a list of length zero does nothing.
 */
function startCountIn(state) {
  const n = Number(state?.startCount);
  return Number.isFinite(n) && n > 0 ? Math.floor(n) : 0;
}

/** The start list's own cursor. WRAPS, and never touches `selected` -- see the mode's header. */
function moveStart(state, delta) {
  const n = startCountIn(state);
  if (n === 0) return { state, toPty: null, action: null };
  const at = Number.isFinite(state.startAt) ? state.startAt : 0;
  return {
    state: { ...state, startAt: (at + delta + n) % n },
    toPty: null,
    action: "start-move",
  };
}
...
function moveMenu(state, delta) {
  const n = menuActionsIn(state).length;
