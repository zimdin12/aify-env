...
export function createNotices({ limit = 20, now = () => Date.now() } = {}) {
  ...
  return {
    ...
    add(message) {
      // ONE ROW, PLAIN TEXT (v0.7 scan, F4). An error can carry an HTTP body -- a proxy's HTML page
      // -- and a newline in a notice split one frame row into several under a writer that addresses
      // rows absolutely. Escapes go too: this ring is drawn on the operator's screen.
      const text = displayable(message, { keepColour: false }).replace(/\s+/g, " ").trim();
      if (!text) return;
      const at = now();
