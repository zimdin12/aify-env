...
import { SHUTDOWN, menuActionsIn } from "./keys.mjs";
// MEASURING AND CUTTING TERMINAL TEXT IS ITS OWN SUBJECT, and it moved out when `width()` had to
// learn East Asian widths. Re-exported because `panes.mjs` and the pane code already import these
// names from here, and one implementation is the whole point.
import { clip, clipToWidth, displayable, pad, width } from "./text-width.mjs";
export { clip, clipToWidth, pad, width };
...
/**
 * What each menu action DOES, in one line (v0.7 scan, F17).
 *
 * The rows were bare verbs, and the confirmation asked "stop alpha?" without saying that stop ends
 * the whole process tree at once, mid-turn. Keyed by the full vocabulary, and a test walks it, so an
 * action added to the menu cannot arrive without saying what it does.
 */
export const ACTION_EFFECTS = Object.freeze({
  attach: "your keys go into its terminal",
  restart: "ends it and starts it again; its context is lost",
  stop: "ends its process tree now — its current turn is lost",
});
...
/**
 * The agents this host could start, as lines.
 *
 * WHAT AN EMPTY LIST MEANS IS THE WHOLE OF THIS FUNCTION'S JUDGEMENT. Three states render
 * identically if nobody separates them, and each sends the operator somewhere different:
 *
 *   not asked yet   the list is being fetched -- say so, or a slow service reads as "none"
 *   nothing to start no agent here can be started -- running already, or refused, and the refusals
 *                    are counted underneath, so the line must not claim more than that.
 *   a problem        aify-comms did not answer, or this host cannot name its own machine. The
 *                    operator should look at the service, not at their agents.
 *
 * IT NAMES THE MACHINE'S SCOPE in the heading, because "no agents" is a claim an operator will
 * check against the dashboard -- where they can see forty. The list is this host's, and a heading
 * that does not say so makes a correct answer look like a broken one.
 *
 * AND IT SAYS WHAT IT REFUSED, for the same reason one line up. The operator, 2026-09-12: "in
 * aify-env start function i see residents for some reason. it should offer only managed options."
 * They already were -- the rule has been there since the list was written -- but a list that shows
 * only what it kept looks exactly like a list with no rule, so there was nothing on screen to
 * disagree with. The refusals are drawn under the offers, counted and grouped by their own reason.
 */
function startList(start, columns, on, window = null) {
  ...
  if (problem) {
    lines.push(`  ${paint(problem, ["yellow"], on)}`);
    // AND STILL DRAW WHATEVER ARRIVED. A partial answer with a warning beside it is more use than
    // a warning alone, and the two are not exclusive: a list can be short AND explained.
  }
  if (!rows.length) {
    if (problem) return [...lines, ...skippedLines(start, columns, on)];
    // UNANSWERED IS NOT EMPTY. `asked` is false until a reply has actually arrived, so a slow or
    // unreachable service cannot be drawn as a host with nothing to start -- which is the false
    // green this project keeps finding, and it would be on screen for as long as the request took.
    lines.push(start?.asked
      // NOT "every agent already has a worker": an offline RESIDENT has none and is refused, and the
      // refusal line underneath says so -- a heading claiming otherwise contradicts it. Found by review.
      ? `  ${paint("nothing to start on this host", ["dim"], on)}`
      // NAMED ONLY IF SOMETHING TOLD US THE NAME. This renderer belongs to the HOST tier, which
      // `docs/AIFY_ENV_BOUNDARY.md` says knows about no service -- and a hardcoded name would be
      // wrong the day a second `aify-` service offers this capability, in the most confusing
      // direction: naming the service that is not the one failing to answer.
      : `  ${paint(start?.service ? `asking ${start.service}…` : "asking…", ["dim"], on)}`);
    return [...lines, ...skippedLines(start, columns, on)];
  }
  // A WINDOW THAT FOLLOWS THE CURSOR when the list is longer than the screen, as the process table
  // does. Drawn whole, a long list pushed the hint line -- and `ctrl+] back` -- off the bottom.
  ...
  for (const [offset, agent] of rows.slice(from, window?.end ?? rows.length).entries()) {
    ...
    // decisions for an operator -- one is a cold agent, the other is one they switched off -- and
    // choosing between them from names alone is choosing blind.
    const detail = [agent.status, agent.runtime].filter(Boolean).join(" · ");
    const row = clip(`${here ? "❯" : " "} ${agent.name || agent.id}${detail ? `  ${detail}` : ""}`, Math.max(10, columns - 4));
    lines.push(`  ${here ? paint(row, ["cyan", "bold"], on) : paint(row, ["dim"], on)}`);
  }
...
function offScreenNote(above, below, on) {
  const parts = [];
...
/**
 * What this host knows and did not offer, under the offers.
 *
 * DRAWN WHETHER THE LIST IS FULL OR EMPTY, which is why it is its own function: "nothing to start"
 * and "thirteen to start" raise the same question about the agents that are missing, and an
 * explanation that only appears in one of those states is the one the operator meets in the other.
 */
...
/**
 * As many hints as fit WHOLE, in display order, dropping the least useful first.
 *
 * THE ONE-GUARANTEE CLIP AT THE END OF THIS FILE CUTS THE TAIL, and for the hint line the tail is
 * where `q quit` lives. Measured 2026-09-08 at the DEFAULT width of 100 columns: adding `s start`
 * took the line to 105 and `q quit` vanished -- so the key that says how to leave was lost to a key
 * that says how to start something, silently, on an ordinary terminal. A hint cut mid-item is worse
 * than one that is shorter: `p…` teaches nothing and looks like a rendering fault.
 *
 * PRIORITY IS NOT DISPLAY ORDER, deliberately. An operator reads left to right and expects movement
 * first, so the ORDER is fixed; what changes under pressure is which items are there at all. The
 * ones that go first are the ones an operator can discover without being told -- arrows, digits,
 * Enter -- and the ones that survive longest are the letters that exist nowhere else on the screen.
 *
 * @param {Array<{text: string, drop: number}>} items display order, `drop` = dropped first when high
 */
function fittedHints(items, columns, separator) {
  const room = Math.max(0, columns - 2);
  const ordered = [...items].sort((a, b) => b.drop - a.drop);
  const dropped = new Set();
  for (;;) {
    const kept = items.filter((item) => !dropped.has(item));
    const text = kept.map((item) => item.text).join(separator);
    if (width(text) <= room || kept.length <= 1) return text;
    // The highest `drop` still present goes next.
    dropped.add(ordered.find((item) => !dropped.has(item)));
  }
}
...
function table(headers, rows, columns, on) {
  ...
  // EVERY CELL IS ONE ROW OF TEXT AND COLOUR (v0.7 scan, F4). A newline in any cell split a frame row
  // under a writer that addresses rows absolutely, and titles and notices were each sanitised at
  // their own producer -- so the next producer nobody sanitised would do it again. Guarded here, once.
  rows = rows.map((row) => row.map((cell) => displayable(cell)));
  ...
  // NO FLOOR ABOVE THE BUDGET. `Math.max(8, ...)` won whenever the fixed columns nearly filled the
  // screen, and the row then ran `2 + fixed + 8` wide regardless -- measured at up to 15 columns
  // past the margin with the operator's real agent names. A table that cannot fit says less rather
  // than spilling: the last column gets whatever is left, even if that is nothing.
  const last = Math.max(0, columns - fixed - 2);
...
function drawDashboard(snapshot, {
  ...
  window = null,
  //: Which slice of the START list to draw, or null for all of it. Set by the same pass.
  startWindow = null,
  //: How many notice rows to draw. The fitting loop lowers this before it touches the process
  //: table, because a burst of notices must not cost the operator the agent list.
  noticeLimit = DEFAULT_NOTICE_ROWS,
  //: Whether this environment ends when its Herdr session does (a `herdr-aify env` instance).
  endsWithHerdr = false,
} = {}) {
  const on = color === true;
  const lines = [];
  // THE TERMINAL'S WIDTH, not a floor under it. `Math.max(60, columns)` rendered a 60-column frame
  // into a 40-column window -- 13 of 19 lines over, every one wrapping, and one wrapped row shifts
  // every row below it out from under `frameUpdate`'s absolute addressing. A 40-column terminal is
  // not exotic; it is a split pane. Narrow is a layout to respect, not an input to override.
  const wide = Math.max(20, columns);
  ...
  const conptyNote = snapshot.terminals?.conptyDll
    ...
    : "";
  // CLIPPED, like every other line. A real node-pty failure reason runs to 91 columns and this line
  // had no budget at all, so it wrapped on any terminal narrower than the message.
  lines.push(clip(snapshot.terminals?.available
    ? `${paint("●", ["green"], on)} terminals available${conptyNote}`
    : `${paint("●", ["red"], on)} terminals UNAVAILABLE ${paint(
      `— ${snapshot.terminals?.reason ?? "unknown"}`, ["dim"], on)}`, wide));
  // WHAT LEAVING DOES, where it stays on screen (v0.7 scan, F21). In a `herdr-aify env` instance a
  // Herdr detach -- harmless in the resident `herdr-aify` -- ends every worker, and the launcher's
  // own warning went to stderr just before the Herdr TUI covered it.
  if (endsWithHerdr) {
    const count = (snapshot.processes ?? []).length;
    lines.push(paint(`! leaving this Herdr session ends this environment and its ${count} worker${count === 1 ? "" : "s"}`,
      ["yellow"], on));
  }
  lines.push("");
  ...
  if (!owned.length && snapshot.answered === false) {
    // NOT EMPTY: UNKNOWN (v0.7 scan, F16). The list is empty because nothing answered, and saying
    // what this environment has or has not started would be a claim about one nobody heard from.
    // `=== false` so a snapshot built without the flag -- older callers, tests -- reads as before.
    lines.push(`  ${paint("?", ["yellow"], on)} ${paint("unknown — the environment is not answering", ["dim"], on)}`);
    ...
    lines.push(...table(
      ...
      // NO PHANTOM COLUMN. An empty header still costs its two-space separator, so a piped render
      // carried a dead column plus its gap on every row.
      // `out` NAMES THE DOT COLUMN, which was headed by an empty string. The operator read a grey
      // circle as "this agent is offline" and reported it as a bug: "claude agents status dot is
      // not green even tho they are online". It was not a bug -- measured 2026-09-08 against the
      // live daemon, every working row had output 0.0-1.6s old across 12 samples and the one grey
      // row had been silent 308 seconds, so the mark was right and the column was mute. A column
      // of symbols with no header invites the reader to supply their own meaning, and liveness is
      // the meaning they will reach for. This one is about OUTPUT: filled means it printed
      // something in the last three seconds, hollow means it went quiet, blank means nothing has
      // been measured yet.
      [...(keys.enabled ? ["#"] : []), "out", "id", "pid", "agent", "service", "io", "up", "title"],
    ...
    // THE ACTIONS MENU, drawn where the operator is already looking: directly under the row it acts
    // on. A modal in the middle of the screen would cover the list they used to choose the agent.
    //
    // IT NAMES ITS TARGET. "stop" with no subject is the shape of every incident where somebody
    // ended the wrong thing, and the row above may have scrolled out of the window by now.
    if (view?.mode === "menu" || (view?.mode === "confirm" && view.confirming !== SHUTDOWN)) {
      // THE BOUND TARGET, NOT THE CURSOR. The menu is about the agent it was OPENED on, and the
      // executor already resolves it that way -- but this did not, so the prompt and the action could
      // name different agents. Review measured it: after a row above it exited, the prompt read
      // "stop charlie?" while `y` stopped bravo. An operator reading the wrong name and confirming is
      // worse than either half alone, because the confirmation is the thing that is supposed to make
      // it safe.
      //
      // FALLING BACK TO THE CURSOR ONLY WHERE NOTHING IS BOUND, which is a menu that has not been
      // opened through the session -- a render test, or a caller that builds a view by hand.
      const bound = view.actionTargetId
        ? (all.find((row) => row?.id === view.actionTargetId) ?? null)
        : (all[view.selected] ?? null);
      // GONE IS SAID, not silently replaced. The executor refuses a target that has disappeared, so
      // the prompt must not offer to stop something else -- and must not read as though it will.
      const name = bound
        ? String(bound.label || bound.id)
        : (view.actionTargetId ? `${view.actionTargetId} (gone)` : "this process");
      if (view.mode === "confirm") {
        // ONE LINE, AND IT ASKS. A confirmation that scrolls is one an operator answers without
        // reading; a confirmation that states the verb and the subject is one they can refuse.
        lines.push(`  ${paint(`${view.confirming} ${name}? `, ["bold", "yellow"], on)}`
          + paint("y to confirm, any other key cancels", ["dim"], on));
        // AND WHAT IT WILL DO, which the verb alone does not say (F17). ITS OWN LINE, so a narrow
        // terminal clips the explanation and never the words that say how to refuse.
        if (ACTION_EFFECTS[view.confirming]) {
          lines.push(`    ${paint(ACTION_EFFECTS[view.confirming], ["yellow"], on)}`);
        }
      } else {
        lines.push(`  ${paint(`actions for ${name}`, ["bold"], on)}`);
        // WHAT THIS CALLER OFFERS, not the vocabulary. A row for an action nobody wired would be
        // a control that does nothing when chosen.
        for (const [at, action] of menuActionsIn(view).entries()) {
          const here = at === (view.menuAt ?? 0);
          const effect = ACTION_EFFECTS[action] ? `  — ${ACTION_EFFECTS[action]}` : "";
          const row = `${here ? "❯" : " "} ${action}${effect}`;
          lines.push(`  ${here ? paint(row, ["cyan", "bold"], on) : paint(row, ["dim"], on)}`);
        }
      }
    }
    ...
    lines.push(...offScreenNote(hiddenAbove, hiddenBelow, on));
  }

  // THE START LIST, drawn under the table like the actions menu and for the same reason: the
  // operator is already looking there. It shows AGENTS, not processes -- rows with no process at
  // all, which is why they cannot appear in the table above and why this list exists.
  //
  // OUTSIDE THE EMPTY CHECK, like the hint below and for a sharper version of its reason: a host
  // running NOTHING is the one most likely to have agents worth starting, so drawing this only
  // beside a populated table would hide the feature exactly where it is wanted. `reconcileFocus`
  // keeps the mode alive through an empty process list for the same reason; a renderer that then
  // drew nothing would make that pointless.
  if (view?.mode === "start") lines.push(...startList(view.start, wide, on, startWindow));

  // THE QUESTION Ctrl+C ASKS IN THE DAEMON'S OWN TERMINAL, drawn whether or not anything is running:
  // it is about the environment, not a row. It says what is at stake, which one keystroke used to
  // end without a word.
  if (view?.mode === "confirm" && view.confirming === SHUTDOWN) {
    const workers = `${owned.length} worker${owned.length === 1 ? "" : "s"}`;
    lines.push(`  ${paint(`stop the environment and its ${workers}? `, ["bold", "yellow"], on)}`
      + paint("y to confirm, any other key cancels", ["dim"], on));
  }
...
if (keys.enabled) {
  ...
  const hints = view?.mode === "confirm"
      // NOTHING ELSE IS OFFERED HERE, because nothing else is accepted: this mode takes three keys
      // and every one that is not `y` cancels. Listing `move` or `find` would be a hint that lies.
      ? [`${key("y")} do it`, `${key("any other key")} cancel`]
      : view?.mode === "menu"
      ? [`${key("↑↓")} choose`, `${key("enter")} run it`, `${key("ctrl+]")} back`]
      : view?.mode === "pty"
      // NOT "keys go to this agent" while they are being dropped (v0.7 scan, F6).
      ? [`${key("ctrl+]")} back to the list`, view?.inputPaused
        ? `${key("input paused")} until the pane shows the agent`
        : `${key("keys")} go to this agent`]
      : view?.mode === "start"
      // ITS OWN LINE, because the keys here are not the dashboard's. An operator reading "1-9 jump"
      // while a list of agents is open would press a digit and watch nothing happen.
      ? [`${key("↑↓")} choose`, `${key("enter")} start it`, `${key("ctrl+]")} back`]
      : view?.mode === "picker"
        ? [`${key("type")} filter`, `${key("↑↓")} move`, `${key("enter")} choose`, `${key("ctrl+]")} cancel`]
      : rows_
        ? [
          // `drop` RISES WITH DISCOVERABILITY. Arrows and digits are how every list works and go
          // first; the lettered features exist nowhere else on the screen and go last.
          `${key("↑↓")} move`,
          `${key("1-9")} jump`,
          `${key("g")} find`,
          // SAYS WHAT PRESSING IT WILL DO, not what the pane currently is. "p console" would leave
          // the operator to guess which way it goes, and the pane defaults to hidden -- so the first
          // thing this hint is ever read for is finding the console, which it should answer outright.
          // NOT WHERE THERE IS NO CONSOLE: inside `herdr-aify env` every worker has its own space.
          ...(view?.withoutConsole ? [] : [
            view?.paneHidden === false ? `${key("p")} hide console` : `${key("p")} show console`,
            `${key("enter")} attach`,
          ]),
          `${key("m")} actions`,
          `${key("s")} start`,
        ]
        // NOTHING SELECTED IS NOT NOTHING TO DO. With no processes the list is empty, so `find` was
        // the only thing offered -- on exactly the host where every agent needs starting. `s` is the
        // one key that acts on something the list is not showing.
        : [`${key("g")} find`, `${key("s")} start`];
    // ONLY WHERE LEAVING IS ON OFFER, and never in a mode that does something else with `q`: in the
    // pane it is the letter q, in a menu it is swallowed so the list behind cannot be acted on, and
    // at a confirmation it is one of the keys that CANCELS. Naming it in those three would be a hint
    // that lies -- which this file's own header says is how an operator learns to stop reading them.
    // FOUR MODES SWALLOW IT NOW. `start` joined them the moment it shipped, and the hint said `q
    // quit` over a list where `q` does nothing -- a lie of exactly the kind this file's header says
    // teaches an operator to stop reading the line. Derived from the modes that ROUTE it rather than
    // typed here, so the next mode to swallow a key cannot get this wrong: see
    // `the-keys-are-documented.test.js`, which presses every key the hint names.
    const qMeansQuit = !["pty", "menu", "confirm", "start"].includes(String(view?.mode ?? ""));
    if (keys.canQuit && qMeansQuit) hints.push(`${key("q")} quit`);
    // WHERE Ctrl+C STOPS EVERYTHING, THE SCREEN SAYS SO. The daemon's own view asks first, but an
    // operator should know before pressing it what the question will be about.
    if (view?.confirmInterrupt === true && String(view?.mode ?? "dashboard") === "dashboard") {
      hints.push(`${key("ctrl+c")} stops everything`);
    }
    // WHOLE ITEMS OR NONE. See `fittedHints`: the clip at the end of this function takes the TAIL,
    // and the tail is where `q quit` sits.
    //
    // The priority is read off the hint's own text rather than carried alongside it, because a
    // parallel array of numbers is a second thing to keep in step with a list that changes per mode.
    // Anything not named here is kept as long as possible, which is the safe default: a new hint is
    // one somebody thought was worth saying.
    const DROP_FIRST = [[" jump", 3], [" move", 2], [" attach", 1]];
    const fitted = fittedHints(
      hints.map((text) => ({
        text,
        drop: (DROP_FIRST.find(([suffix]) => text.endsWith(suffix)) ?? [null, 0])[1],
      })),
      wide,
      paint("  ·  ", ["grey"], on),
    );
    lines.push(`  ${paint(fitted, ["dim"], on)}`);
  }

  if (snapshot.unknown?.length) {
    lines.push("");
    lines.push(`  ${paint(
      `${snapshot.unknown.length} process${snapshot.unknown.length === 1 ? "" : "es"} whose liveness could not be determined:`,
      ["yellow"], on)}`);
  ...
  //
  // AND THEY YIELD SPACE BEFORE THE PROCESSES DO. The fitting loop below used to shrink only the
  // process table, so twenty notice rows -- which one failing poll produces in seconds -- pushed a
  // whole fleet down to a single line. The operator's words: "i would rather see more agents and less
  // notices, most useful thing in this is that i see what agent is working and what not." A repeating
  // failure is already one row with an `x38` on it; the agents are the reason the view is open.
  const allNotices = Array.isArray(snapshot.notices) ? snapshot.notices : [];
  const noticeRoom = Math.max(0, Math.floor(noticeLimit) || 0);
  // NEWEST FIRST, so the cap keeps the ones being asked about rather than the oldest.
  const notices = [...allNotices].reverse().slice(0, noticeRoom);
  if (notices.length) {
    lines.push("");
    lines.push(heading(
      "NOTICES",
      allNotices.length > notices.length
        ? `${notices.length} of ${allNotices.length}`
  ...
  // ONE GUARANTEE AT THE END, rather than a budget on each of a dozen line kinds.
  //
  // Clipping line by line left whole classes unbudgeted -- the title, the health lines, the hint
  // that teaches the keys, the traffic footer -- and the TABLE could overflow even when every cell
  // was clipped, because its FIXED columns alone can exceed a narrow terminal. Measured at 40
  // columns: six lines over, from five different code paths.
  //
  // A line wider than the terminal wraps, and one wrapped row shifts every row below it out from
  // under `frameUpdate`'s absolute addressing -- so this is not cosmetic, it is the difference
  // between a diff that describes the screen and one that does not. Chasing each producer would
  // leave the next one to be found by an operator.
  return lines.map((line) => clip(line, wide));
}
 ...
 */
//: How many notice rows a full-height view shows. TEN, because a glance panel is for noticing that
//: something is wrong, not for reading the history -- the ring behind it still holds twenty, and a
//: repeating failure is one row with a count on it either way. The operator asked for this number
//: after twenty rows left a three-agent fleet showing one.
export const DEFAULT_NOTICE_ROWS = 10;
...
export function renderDashboard(snapshot, options = {}) {
  ...
  const start = options.view?.mode === "start" ? options.view.start : null;
  const starting = Array.isArray(start?.agents) ? start.agents.length : 0;

  // NOTICES GIVE WAY FIRST, AND ON EVERY HOST (v0.7 scan, F12). The early return for a list too short
  // to window used to sit above this loop, so an idle host -- the common case -- never trimmed its
  // notices, and the frame ran past the terminal and lost TRAFFIC off the bottom.
  //
  // They used to be untouchable while the process table absorbed the whole
  // overflow, so a burst of `output not delivered: fetch failed` -- which one restart produces in
  // seconds -- cost the operator every agent row but one. The agents are what the view is for.
  let noticeLimit = Number.isFinite(options.noticeLimit)
    ? Math.floor(options.noticeLimit)
    : DEFAULT_NOTICE_ROWS;
  let out = drawn;
  while (out.length > limit && noticeLimit > 0) {
    noticeLimit = Math.max(0, noticeLimit - (out.length - limit));
    out = drawDashboard(snapshot, { ...options, noticeLimit });
  }
  if (out.length <= limit) return out;
  // Only the table-windowing passes below need more than one row to window.
  if (all.length <= 1 && starting <= 1) return out;
  ...
  for (let attempt = 0; all.length > 1 && attempt < 4 && out.length > limit; attempt += 1) {
    keep = Math.max(1, keep - (out.length - limit));
    window = windowAround(options.view?.selected ?? 0, all.length, keep);
    out = drawDashboard(snapshot, { ...options, noticeLimit, window });
  }
  // THEN THE START LIST, the same way. It comes last because it is what the keys are moving through
  // in that mode, so it keeps its rows longest.
  let keepStart = starting;
  for (let attempt = 0; starting > 1 && attempt < 4 && out.length > limit; attempt += 1) {
    keepStart = Math.max(1, keepStart - (out.length - limit));
    out = drawDashboard(snapshot, {
      ...options, noticeLimit, window, startWindow: windowAround(start.at ?? 0, starting, keepStart),
    });
  }
  return out;
}
