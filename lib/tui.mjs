 ...
 *   nothing to start every agent this host knows already has a worker. The healthy answer.
...
function startList(start, columns, on) {
  ...
  if (!rows.length) {
    ...
    lines.push(start?.asked
      ? `  ${paint("nothing to start — every agent this host knows already has a worker", ["dim"], on)}`
