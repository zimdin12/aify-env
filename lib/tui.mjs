...
function drawDashboard(snapshot, {
  ...
  if (!owned.length) {
    ...
    if (view?.mode === "menu" || view?.mode === "confirm") {
      // FROM THE FULL LIST, not the visible slice. `shown` is windowed around the selection, so
      // indexing it with an absolute `selected` names the wrong agent the moment the table scrolls --
      // which is the row-shift defect again, on the one screen whose whole job is to say what it is
      // about to stop.
      const target = all[view.selected] ?? null;
      const name = String(target?.label || target?.id || "this process");
