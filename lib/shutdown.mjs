 ...
 *   beforeStop?: () => unknown,   // awaited, bounded by `beforeStopBudgetMs`
 * }} deps
...
//: How long `beforeStop` may take before the stops begin anyway. It is AWAITED, because the plugins'
//: stop is a round trip to a service -- an offline heartbeat saying this host holds no terminal any
//: more, and the exit markers still on their way -- and a daemon that exits in the same turn cuts
//: both off. After a restart the service then knew nothing about the predecessor's terminals.
//:
//: TWO SECONDS: a loopback round trip is milliseconds, and this sits in front of the five-second stop
//: deadline an operator is already waiting through. A service that does not answer inside it is not
//: one the teardown can wait for.
const BEFORE_STOP_BUDGET_MS = 2000;


export function createShutdown({
  runner, closeServer, clearOwned, exit, write = () => {}, beforeStop = () => {},
  stopDeadlineMs = STOP_DEADLINE_MS, beforeStopBudgetMs = BEFORE_STOP_BUDGET_MS,
  setTimer = setTimeout, clearTimer = clearTimeout,
}) {
  let shuttingDown = false;
  //: Set while `beforeStop` is being waited for: a second signal then ends the WAIT, not the daemon.
  let skipWait = null;
  return async function shutdown(signal) {
    ...
    //
    // EXCEPT WHILE `beforeStop` IS STILL BEING WAITED FOR. That wait is the one window before the
    // stops are issued, and exiting inside it would leave every agent running: the second signal
    // ends the wait instead, and the stops follow at once. (A gate on this was once removed as
    // decoration, when `beforeStop` was not awaited and the window did not exist. It exists now, and
    // `a second signal WHILE WAITING skips the wait` fails without this branch.)
    if (shuttingDown && skipWait) {
      write(`[aify-env] ${signal} again: not waiting for the service any longer; stopping processes now\n`);
      skipWait();
      return;
    }
    if (shuttingDown) {
...
`);
    ...
    //
    // AWAITED UNDER A BUDGET, never unbounded: whichever comes first -- it settles, the budget runs
    // out, or a second signal arrives. Anything it does synchronously still happens before the first
    // await, which is what the daemon's view relies on to release the terminal.
    let budgetTimer = null;
    try {
      await Promise.race([
        Promise.resolve(beforeStop()).catch(() => {}),
        new Promise((resolve) => {
          skipWait = resolve;
          budgetTimer = setTimer(resolve, beforeStopBudgetMs);
        }),
      ]);
    } catch {
    ...
    // HOW FAR EACH ONE GOT. One shared map rather than a local per stop, because the timeout below
    // has to read it after `Promise.race` has thrown the stop promises away.
    const reached = new Map(owned.map((process_) => [process_.id, "starting"]));
...
`);
      ...
      let settled;
      try {
        // THE PHASE LINE IS WRITTEN BEFORE THE CALL IT NAMES, which is the only ordering that
        // survives a SYNCHRONOUS wedge: `child.kill()` on ConPTY can hold the loop, and then
        // neither the deadline nor any line after the call ever runs. What is already on screen is
        // all the operator will ever have.
        settled = Promise.resolve(runner.stop(process_.id, {
          phase: (name) => {
            reached.set(process_.id, name);
            if (name !== "done") write(`[aify-env] ${signal}: ${process_.id} ${name}\n`);
          },
        }));
      } catch (thrown) {
    ...
    if (stuck.length) {
      ...
      // NAMED WITH THE CALL EACH ONE IS SITTING IN. "p2 did not confirm" says a process is stuck;
      // "p2 (in console-kill)" says WHICH of the two blocking calls, which is the difference
      // between a report somebody can act on and another round of guessing.
      const where = stuck.map((id) => `${id} (in ${reached.get(id) ?? "starting"})`).join(", ");
      write(`[aify-env] ${signal}: ${where} did not confirm within ${stopDeadlineMs}ms; `
