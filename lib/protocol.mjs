...
import { inputPayload } from "./input-encoding.mjs";
...
const ROUTES = [
  {
    ...
    handle: (_id, _request, deps) => ok(200, {
      ...
      // WHERE KEYSTROKES CAN GO FASTER, advertised rather than guessed. Empty means this daemon has
      // no local socket -- switched off, older, or it failed to listen -- and a client uses HTTP.
      // A client on another host or in WSL cannot open it whatever it says here, which is why the
      // client always keeps the HTTP path it arrived on.
      inputSocket: deps.inputSocket ?? "",
  ...
  },
  {
    ...
    handle: (id, request, deps) => {
      if (!isPlainObject(request.body)) return error(400, "an input request must carry a string `data`");
      // WHAT THE BYTES MEAN IS THE SENDER'S TO SAY. A raw-stdin client sends latin1 bytes and the
      // dashboard sends text; writing both as text double-encodes one of them. See input-encoding.mjs.
      const payload = inputPayload(request.body);
      if (!payload.ok) return error(400, payload.error);
      const written = deps.runner.write(id, payload.value);
  ...
  // ── STARTING A KNOWN AGENT ───────────────────────────────────────────────────────────────────
  //
  // WHY THE ROUTES EXIST AT ALL, given the client could ask aify-comms itself: it has no credential
  // and no endpoint. The service plugin holds both, this daemon holds the plugin, and the TUI is a
  // client on the other side of loopback -- so the capability lives where the credential does and
  // both tiers reach one implementation. The alternative is a second credential resolver in a
  // process whose whole job is drawing a screen.
  //
  // A DAEMON WITH NO SUCH PLUGIN ANSWERS 503, not an empty list. Those two render identically and
  // one of them means the operator should look at their plugin configuration rather than at their
  // agents. The body still carries `agents` and `problem` in both cases, so a view has one shape to
  // render whatever happened.
  {
    name: "/agents/startable",
    methods: ["GET"],
    match: (path) => (path === "/agents/startable" ? "" : null),
    handle: async (_id, _request, deps) => {
      const agents = deps.agents;
      // THE SAME SHAPE ON BOTH PATHS, `service` included. A field present on one status and absent on
      // the other is two shapes for a view to render, which is the thing this route's own comment
      // promises it will not be -- and the reader would meet the difference only on the failing path.
      if (!agents) {
        return ok(503, { agents: [], skipped: [], problem: "no service plugin on this host can start agents", service: "" });
      }
      const answer = await agents.list();
      // THE CAPABILITY NAMES ITSELF; this daemon does not name it. Empty when the capability does not
      // say, which a view renders as an unattributed wait rather than as a guess.
      return ok(200, {
        agents: answer?.agents ?? [],
        //: WHAT IT REFUSED, so a view can say why a known agent is absent. Same shape on both paths.
        skipped: answer?.skipped ?? [],
        problem: String(answer?.problem || ""),
        service: String(agents.service || ""),
      });
    },
  },
  {
    name: "/agents/:id/start",
    methods: ["POST"],
    match: (path) => captureIdUnder(path, "/agents/", "/start"),
    // THE STATUS AND THE BODY AGREE, deliberately. A refusal is an ordinary answer here -- the agent
    // came up in between, it has no session, the service said no -- and it carries a reason no HTTP
    // code can express. Answering 200 with `started: false` would read as success to a caller that
    // checks only `response.ok`; answering 409 with no body would lose the reason. So both travel,
    // and a caller reading either signal alone still gets the right answer.
    handle: async (id, _request, deps) => {
      const agents = deps.agents;
      if (!agents) {
        return ok(503, { started: false, agentId: id, sessionId: "", problem: "no service plugin on this host can start agents" });
      }
      const result = await agents.start(id);
      return ok(result?.started ? 200 : 409, {
        started: Boolean(result?.started),
        agentId: String(result?.agentId || id),
  ...
  },
  {
    ...
    handle: async (id, _request, deps) => {
      ...
      // been second. But a process that SURVIVED its kill is not stopped, and saying 204 about it
      // told `aify-env tui` to report "stopped" (v0.7.1 review, E8), so that one answers 500.
      const { stopped, problem } = await stopAndVerify(deps.runner, id, deps.isAlive ? { isAlive: deps.isAlive } : {});
      return stopped ? { status: 204, body: null } : { status: 500, body: { stopped: false, problem } };
    },
  },
];
...
/**
 * `<prefix><id><suffix>` -> id, or null.
 *
 * AN EMPTY ID IS NOT A MATCH, which is the rule `captureId` above already carries and the reason
 * this is a function rather than two inline slices: `/agents//start` would otherwise reach a handler
 * with an empty name, and every guard downstream would have to remember to refuse it.
 */
function captureIdUnder(path, prefix, suffix) {
