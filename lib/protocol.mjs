...
async function startProcess(body, deps) {
  ...
  const handle = await deps.runner.start({
    ...
    args: plan.args,
    // WHAT WAS ASKED FOR, kept beside how it will be run. The record the reaper later verifies against
    // has to name THIS process, and `plan.command` is `bash.exe` for any shebang launcher on Windows
    // -- a name shared with every other bash-launched process on the host. The launcher path is the
    // discriminating half, and it is in the spawned command line either way (interpreterFor puts it in
    // `args`), so verification against it matches exactly the process we started.
