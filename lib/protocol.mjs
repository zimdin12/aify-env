...
export async function handleRequest(request, deps) {
  ...
  if (path.endsWith("/output") && path.startsWith("/processes/")) {
    const id = path.slice("/processes/".length, -"/output".length);
    if (method !== "GET") return error(405, `${method} is not supported on /processes/:id/output`);
    // Answered here, streamed by the transport. A caller must be able to tell "no such process" from
    // "a process that has printed nothing yet": one is a 404, the other is an open stream that is
    // simply quiet, and conflating them makes a console look broken for a reason nobody can see.
    if (!deps.runner.canStream(id)) return error(404, `no such process: ${id}`);
    return { status: 200, body: null, stream: id };
  }

  if (path.endsWith("/input") && path.startsWith("/processes/")) {
    const id = path.slice("/processes/".length, -"/input".length);
    if (method !== "POST") return error(405, `${method} is not supported on /processes/:id/input`);
  ...
  if (path.endsWith("/resize") && path.startsWith("/processes/")) {
    const id = path.slice("/processes/".length, -"/resize".length);
    if (method !== "POST") return error(405, `${method} is not supported on /processes/:id/resize`);
    ...
    // apply to it, and a console must be able to tell those apart before it decides to retry.
    if (!resized.ok) {
      const missing = /no such process/i.test(resized.error);
      return error(missing ? 404 : 409, resized.error);
    }
    return { status: 204, body: null };
  }
