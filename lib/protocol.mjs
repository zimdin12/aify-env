...
const ROUTES = [
  ...
  // render whatever happened.
  {
    ...
    handle: async (_id, _request, deps) => {
      ...
      if (!agents) return ok(503, { agents: [], problem: "no service plugin on this host can start agents" });
      const answer = await agents.list();
      return ok(200, { agents: answer?.agents ?? [], problem: String(answer?.problem || "") });
    },
  },
  {
