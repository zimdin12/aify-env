...
export class InputSender {
  ...
  /**
   * Whether everything queued was sent within `ms`. For a detach: long enough for a keystroke in
   * flight to land, bounded so a dead daemon cannot hold the operator's terminal.
   */
  async drainedWithin(ms) {
    if (!this.busy) return true;
    let timer = null;
    const late = new Promise((resolve) => { timer = setTimeout(() => resolve(false), Math.max(0, ms)); });
    try {
      return await Promise.race([this.drained().then(() => true), late]);
    } finally {
      clearTimeout(timer);
    }
  }
