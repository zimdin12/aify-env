 ...
 *
 * @returns {Promise<{agents: Array<object>, skipped: Array<object>, problem: string}>}
 */
  ...
  try {
    ...
    if (body && typeof body === "object") {
      return {
        ...
        //: WHAT THE HOST REFUSED. Dropped here, a `tui` client would draw the same unexplained list
        //: the daemon's own view explains -- one question with two answers depending on who asked.
        ...
        //: Passed through, never defaulted. This process has no idea which service is behind the
        //: daemon and must not invent one.
        service: String(body.service || ""),
      };
    }
    return { agents: [], skipped: [], problem: `the environment answered ${response.status} with no list` };
  } catch (error) {
    return { agents: [], skipped: [], problem: `the environment did not answer: ${error?.message || error}` };
  }
}
 ...
 * stopped answering.
 *
 * THE DAEMON'S REASON TRAVELS (v0.7.1 review, E8). A stop whose process survived the kill answers
 * 500 with `{problem}`, and that sentence -- which pid is still running -- is what the operator needs.
 *
 * @returns {Promise<{ok: boolean, problem: string}>} whether the daemon stopped it, and why not
 */
export async function performClientAction({ action, process: target }, { endpoint, fetchImpl = fetch } = {}) {
  const refused = (problem) => ({ ok: false, problem });
  if (action !== "stop") return refused(`${action} is not something this client can do`);
  const id = String(target?.id ?? "");
  if (!id) return refused("no process was named");
  const base = String(endpoint || "").replace(/\/+$/, "");
  try {
    ...
    if (response?.ok) return { ok: true, problem: "" };
    const body = await Promise.resolve(response?.json?.()).catch(() => null);
    return refused(String(body?.problem || `the environment answered ${response?.status ?? "nothing"}`));
  } catch (error) {
    return refused(`the environment did not answer: ${error?.message || error}`);
  }
}
