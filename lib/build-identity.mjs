 ...
 * @param {(dir: string) => string[]} list  a directory reader returning bare filenames
 * @param {(...parts: string[]) => string} join
 * @returns {string[]} paths, unsorted
 */
export function sourceFiles(root, list, join) {
  const lib = list(join(root, "lib")).filter((file) => file.endsWith(".mjs") || file.endsWith(".js"));
  return [
    // The entry point counts. It holds the wiring, the routes' dependency bag and the signal
    // handlers -- a change there is a change to the program even when every module is untouched.
    join(root, "bin", "aify-env.mjs"),
    ...lib.map((file) => join(root, "lib", file)),
  ];
}
