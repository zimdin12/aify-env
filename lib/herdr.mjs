...
export function herdrCandidates(env = process.env, { platform = process.platform, home = homedir(), readdir = listDir } = {}) {
  ...
  if (windows) {
    ...
    // BY VERSION, NOT BY SPELLING (v0.7.1 review, E7): as strings, 0.10.0 sorts below 0.9.0.
    const newestFirst = (a, b) => String(b).localeCompare(String(a), undefined, { numeric: true });
    for (const release of [...readdir(path.join(standalone, 'releases'))].sort(newestFirst)) {
