...
/**
 * What the producer is, told BEFORE the replay: `cols`, `rows`, `truncated`, `resized`,
 * `replayBytes`, and `checkpoint` when what follows is the daemon's serialized screen instead.
 *
...
export function parseFrame(text) {
  ...
  if (event === FRAME_META) {
    ...
    return {
      ...
      // AND FAILS CLOSED THE SAME WAY, for the same reason and with the opposite default. A frame
      // that does not say whether the PTY was resized has not said the geometry held; a daemon too
      // old to send the field is exactly the daemon whose resizes nobody was tracking. Absent, a
      // string or null all become TRUE, so the consumer waits for a repaint rather than replaying
      // bytes into a size they were never written at.
      resized: typeof decoded.resized === "boolean" ? decoded.resized : true,
      replayBytes: Number(decoded.replayBytes) || 0,
      // THE BYTES THAT FOLLOW ARE A SERIALIZED SCREEN, not a replay (`screen-checkpoint.mjs`). The
      // one field here that grants soundness, so only a literal `true` counts and silence is `false`.
      checkpoint: decoded.checkpoint === true,
    };
  }
...
/**
 * How often a quiet stream says it is still open.
 *
 * MEASURED 2026-09-14: Node's fetch aborts a response body after 300s with no bytes
 * (`UND_ERR_BODY_TIMEOUT`), and the daemon wrote nothing while a process was quiet. An idle Claude Code
 * worker's `aify-env attach` pane died at 305s; a hermes pane beside it, redrawing every ~1.7s, lived.
 * Thirty seconds leaves room for many missed beats before that deadline.
 */
export const KEEPALIVE_MS = 30_000;

/** An SSE comment. It carries no `data:`, so `parseFrame` returns null for it and EventSource skips it. */
export const KEEPALIVE_FRAME = ": keepalive" + FRAME_END;

/**
 * Beat on an open event-stream response until it closes. The one writer here that holds a timer.
 *
 * STOPS ON `close`, which fires for every way a stream ends -- the process exiting, the route racing
 * a dead process, a viewer leaving -- so the call site needs no teardown of its own. It also refuses
 * to write once `end()` has been called, because `close` can arrive a tick after it.
 */
export function keepStreamAlive(response, { everyMs = KEEPALIVE_MS, timers = globalThis } = {}) {
  const timer = timers.setInterval(() => {
    if (!response.writableEnded && !response.destroyed) response.write(KEEPALIVE_FRAME);
  }, everyMs);
  timer?.unref?.();
  const stop = () => timers.clearInterval(timer);
