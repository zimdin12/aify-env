...
/**
 * How many rows of this screen carry anything: up to and including the last one that is not blank.
 *
 * THE PANE KEEPS THE LAST `height` OF THESE, so this -- not the PTY's height -- is what decides
 * whether any painted row is out of view (v0.7.1 review, E4).
 */
export function paintedRowCount(rows) {
  const all = Array.isArray(rows) ? rows : [];
  let last = all.length;
  while (last > 0 && isBlankRow(all[last - 1])) last -= 1;
  return last;
}
...
export function screenLines(rows, { width = 80, height = 24 } = {}) {
  ...
  const all = Array.isArray(rows) ? rows : [];
  // A CROP of a bigger picture rather than a screen that happened to fit. The emulator runs at the
  // PRODUCER's geometry, never the pane's, and the pane takes what it can show. Sizing it to the pane
  // instead was the first design here and review killed it with a measurement: identical bytes at 80
  // and 40 columns do not merely clip differently, they WRAP -- a row-1 overflow lands on row 2 and
  // collides with the text that belongs there, so a narrow emulator does not show a smaller screen,
  // it shows a different and wrong one.
  //
  // TRAILING BLANKS GO FIRST, then THE LAST `height` PAINTED ROWS are kept (v0.7 scan, F5). A coding
  // agent's live edge -- its prompt, its status line, its input box -- is at the BOTTOM of what it
  // painted. Cropping from the top dropped exactly those rows on an ordinary terminal (a 30-row PTY
  // in a 22-row pane), so an attached pane said "typing here" while the line being typed into was off
  // screen. Measuring from the last painted row rather than the last PTY row keeps a short screen at
  // the top of a tall PTY whole: its empty bottom costs nothing.
  const last = paintedRowCount(all);
  const visible = all.slice(Math.max(0, last - limit), last);
  ...
  return visible.map((row) => clipToWidth(String(row ?? ""), cols));
}
