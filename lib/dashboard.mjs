...
export async function startDashboard({
  ...
  // neither.
  const console_ = input ? new ConsoleSession({ endpoint, fetchImpl, actions }) : null;
  ...
  const draw = async () => {
    ...
    const drewAPane = paneWillBeDrawn(console_?.pane() ?? null, liveColumns);
    try {
      if (clearScreen) {
  ...
  if (input) {
    ...
    onData = (chunk) => {
      ...
      try {
        const { quit, interrupt, toPty, action, perform } = console_.handleInput(String(chunk));
