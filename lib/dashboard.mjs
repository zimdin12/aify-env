...
import { ENTER_VIEW, LEAVE_VIEW, frameUpdate } from "./frame.mjs";
...
import { actionOutcomeNotice, startOutcomeNotice } from "./action-outcome.mjs";
...
export async function collectSnapshot({
  ...
  return {
    //: WHETHER THE ENVIRONMENT ANSWERED AT ALL (v0.7 scan, F16). Every field below has a default for
    //: when it did not, and a renderer that cannot tell a default from an answer states facts about
    //: an environment nobody heard from.
    answered: env.ok === true,
...
export async function startDashboard({
  ...
  onInterrupt = null,
  //: WHETHER Ctrl+C ON THE DASHBOARD ASKS FIRST. Off by default, which is the client's case; the
  //: daemon turns it on, because there `onInterrupt` ends every worker on the host.
  ...
  //: Whether this environment ends with the Herdr session it runs in, so the header can say so.
  endsWithHerdr = false,
  onInput = null,
  //: WHOSE `exit` GIVES THE SCREEN BACK. The process, always, outside a test.
  ...
  const console_ = input
    ? new ConsoleSession({
      endpoint, fetchImpl, actions, withoutConsole, confirmInterrupt, onProgress: notePaneProgress,
    })
    : null;
  ...
  let stopped = false;
  //: Whether this view has switched to its own screen, so `stop()` gives it back exactly once.
  let enteredView = false;
  //: The process is ending with this view still up: stop it. `stop` is defined last, below, and a
  //: frame can be written before then, so this reads it when the exit comes.
  let stopView = null;
  const releaseOnExit = () => stopView?.();
  ...
  const render = (snapshot) => {
    ...
    const lines = composeConsole({
      ...
      dashboardLines: renderDashboard(snapshot, {
        ...
        view: console_ ? {
          ...
          withoutConsole: console_.withoutConsole,
          ...
          //: Why an attached pane is dropping keys, so the hint agrees with the pane's title.
          inputPaused: console_.pane()?.inputPaused ?? "",
          //: So the screen says what Ctrl+C does where it stops everything.
          confirmInterrupt: console_.focus.confirmInterrupt === true,
    ...
    try {
      if (clearScreen) {
        // A LIVE VIEW DRAWS ON A SCREEN OF ITS OWN, with the cursor hidden (v0.7 scan, F13), and
        // `stop()` hands the operator's screen back. Not for a one-shot render: that frame is the
        // output, and it has to still be there after the command exits.
        let bytes = frameUpdate(previousLines, lines);
        if (!once && !enteredView && bytes) {
          bytes = `${ENTER_VIEW}${bytes}`;
          enteredView = true;
          // AND IT IS GIVEN BACK HOWEVER THE PROCESS ENDS (v0.7.1 review, E2). `exit` is emitted for
          // an uncaught exception or rejection too, which no caller's handler saw; `stop` is
          // synchronous, which is all an `exit` listener gets to be.
          processEvents?.on?.("exit", releaseOnExit);
        }
  ...
  //
  // ONE COLLECTION AT A TIME (v0.7 scan, F26). Service probes run in sequence with their own
  // timeouts, so a collection can outlast the interval; a tick that started another beside it let an
  // older snapshot paint over a newer one. A tick that finds one still running is skipped.
  let collecting = false;
  const timer = setInterval(() => {
    if (collecting) return;
    collecting = true;
    draw().catch(() => {}).finally(() => { collecting = false; });
  }, intervalMs);
  ...
  let onData = null;
  //: The timer that lets held-back input go, and what arms it. Set up with the keyboard, below.
  let inputFlushTimer = null;
  let scheduleInputFlush = () => {};
  const rawWasOn = Boolean(input?.isRaw);
  //: One outcome line into NOTICES, drawn at once. Nothing when there is no ring or nothing to say.
  const report = (text) => {
    if (!text || !notices || stopped) return;
    notices.add(text);
    repaint();
  };
  if (input) {
    ...
    input.resume?.();
    //: What one handled key asks of the caller. True when the view is being left, so the rest of the
    //: read is not acted on.
    const actOn = ({ quit, interrupt, toPty, action, perform, startAgent }) => {
      ...
      //
      // ONLY THE ANSWER TO THIS OPENING (v0.7.1 review, W12): one that lands after a reopen is dropped.
      if (action === "start-open" && onStartList) {
        const opening = console_.startOpening;
        const current = () => console_.startOpening === opening;
        Promise.resolve(onStartList())
          .then((answer) => {
            if (!current()) return;
            console_.noteStartable(answer?.agents, answer?.problem, answer?.service, answer?.skipped);
            repaint();
          })
          .catch((error) => {
      ...
      //
      // AND WHAT HAPPENED IS REPORTED (v0.7 scan, F9). The answer used to be dropped, so a start
      // the service refused looked exactly like one that was on its way.
      if (startAgent && onStartAgent) {
        Promise.resolve(onStartAgent(startAgent))
          .then((result) => report(startOutcomeNotice(startAgent, result)))
          .catch((error) => report(startOutcomeNotice(startAgent,
            { started: false, problem: String(error?.message || error) })));
      }
      ...
      //
      // CALLED SYNCHRONOUSLY, as it always was, and its answer reported like a start's (F9).
      if (perform && onAction) {
        let outcome;
        try {
          outcome = onAction(perform);
        } catch (error) {
          outcome = { ok: false, problem: String(error?.message || error) };
        }
        Promise.resolve(outcome)
          .then((result) => report(actionOutcomeNotice(perform, result)))
          .catch((error) => report(actionOutcomeNotice(perform,
            { ok: false, problem: String(error?.message || error) })));
      }
    ...
    // INPUT HELD BACK FOR THE REST OF A PASTE MARKER is let go once the terminal goes quiet (v0.7.1
    // review, T06). Rescheduled on every read, so "quiet" is measured from the last one.
    scheduleInputFlush = () => {
      if (inputFlushTimer) clearTimeout(inputFlushTimer);
      inputFlushTimer = null;
      const wait = console_.inputFlushMs;
      if (wait === null || stopped) return;
      inputFlushTimer = setTimeout(() => {
        inputFlushTimer = null;
        if (stopped) return;
        try {
          for (const result of console_.flushInput()) if (actOn(result)) return;
        } catch {
          // As for a read: a key that could not be handled is not worth the screen.
        }
        repaint();
      }, wait);
      inputFlushTimer.unref?.();
    };
    ...
    onData = (chunk) => {
      ...
      try {
        // DECODED ACROSS READS (v0.7 scan, F23). `String(chunk)` decoded each read on its own, so a
        // character whose bytes straddled two reads -- a large non-ASCII paste -- reached the agent
        // as two replacement characters. One decoder holds the partial bytes until the rest arrives.
        const text = typeof chunk === "string" ? chunk : decoder.write(chunk);
        if (!text) return;
        // ONE READ MAY CARRY SEVERAL KEYS, handled in order (v0.7 scan, F14). See `handleChunk`.
        for (const result of console_.handleChunk(text)) {
          if (actOn(result)) return;
        }
      } catch {
  ...
  const view = {
    ...
    stop: () => {
      ...
      console_?.stop();
      // THE OPERATOR'S SCREEN AND CURSOR COME BACK, synchronously, like raw mode below: a daemon
      // prints its shutdown onto the screen it was started from.
      if (enteredView) {
        enteredView = false;
        try { write(LEAVE_VIEW); } catch { /* a terminal that is gone needs nothing back */ }
      }
      if (input && onData) {
      ...
      processEvents?.off?.("exit", releaseOnExit);
    },
  };
  stopView = view.stop;
  return view;
