...
//              because two that disagree is a defect this repo has already removed once. Since
//              v0.7 it ASKS first ("stop the environment and its N workers?", a single `y`), and
//              in the menu, the start list and the picker it backs out instead.
...
// `createShutdown`. `shutdown.mjs` called its `beforeStop` WITHOUT awaiting it, while the daemon
// passes an ASYNC callback, so anything after the first `await` in there ran only if the process
// survived to run it. It now awaits it only up to a budget, and a second signal ends the wait, so
// the tail can still be cut off: `exit` is `process.exit(0)`.
//
...
//
// The fix is an ORDERING in `bin/aify-env.mjs`: release the screen before the first await, so it
// happens whatever the budget or a second signal does to the rest of the callback.
...
export async function startDaemonView({
  ...
  //: needed.
  agents = null,
  // WHETHER THIS DAEMON OPENS A HERDR SPACE FOR EVERY WORKER. There the view's console and attach are
  // a second, worse way to reach an agent that already has a space of its own, so neither is offered.
  // Not `HERDR_ENV`: a daemon started from an ordinary Herdr pane opens no spaces and needs both.
  herdrSpaces = false,
  notices = null,
  ...
  enabled = true,
  intervalMs = 2000,
  //: HOW OFTEN ARRIVING OUTPUT MAY REDRAW THE PANE, forwarded for the same reason `intervalMs` is:
  //: the daemon owns this terminal and the operator tunes it through the daemon. Undefined here
  //: leaves `startDashboard`'s own default in place rather than pinning a second copy of it.
  paneRepaintMs = undefined,
  color = true,
  write = (line) => process.stderr.write(line),
  start = startDashboard,
  //: How a stopped pid is checked. Injected so a test can make a process survive its kill.
  isAlive = undefined,
} = {}) {
  ...
  try {
    const view = await start({
      ...
      registryPath,
      intervalMs,
      ...(paneRepaintMs === undefined ? {} : { paneRepaintMs }),
      columns: stdout.columns || 100,
      color,
      notices,
      rows: stdout.rows || 24,
      // A DAEMON THAT OPENS A SPACE PER WORKER RUNS IN A `herdr-aify env` INSTANCE, whose lifetime is
      // the Herdr session's: the header says leaving ends every worker (v0.7 scan, F21).
      endsWithHerdr: herdrSpaces === true,
      ...
      ...(stdin?.isTTY ? {
        ...
        // AND IT ASKS FIRST. One Ctrl+C ended every managed worker on the host; the view now asks
        // "stop the environment and its N workers?" and only a single `y` reaches `onInterrupt`.
        ...
        actions: herdrSpaces ? ["stop"] : ["attach", "stop"],
        withoutConsole: herdrSpaces,
        ...
        // THE ANSWER IS HANDED BACK, never dropped (v0.7 scan, F9): the view reports a refusal and
        // its reason, which is the difference between "refused" and "on its way".
        onStartAgent: async (agent) => {
          const capability = agents?.();
          if (!capability) return { started: false, problem: "no service plugin on this host can start agents" };
          return capability.start(agent?.id);
        },
        onAction: ({ action, process }) => {
          ...
          if (action === "stop" && process?.id) {
            // NOT AWAITED by the view: this runs inside a `data` listener, and a stop that takes
            // seconds must not hold the keyboard. The OUTCOME is handed back rather than swallowed,
            // and the view reports it (F9); a rejection becomes `{ok: false}`, never an unhandled one.
            // "STOPPED" ONLY ONCE THE PID IS GONE (v0.7.1 review, E8): `runner.stop` resolving says
            // the kill was sent, not that it took.
            return stopAndVerify(runner, process.id, isAlive ? { isAlive } : {})
              .then(({ stopped, problem }) => ({ ok: stopped, problem }),
                (error) => ({ ok: false, problem: String(error?.message || error) }));
          }
          return undefined;
        },
      } : {}),
    ...
    const onResize = () => { view.resize?.({ columns: stdout.columns, rows: stdout.rows }); };
    stdout?.on?.("resize", onResize);
    // AND REMOVED WITH THE VIEW (v0.7 scan, F26). Returning the dashboard's stop alone left this
    // listener on stdout for good -- harmless while stop happens only at exit, a leak otherwise.
    return {
      stop: () => {
