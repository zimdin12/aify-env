...
/**
 * Move a record we could not understand ASIDE, rather than writing over it.
 *
 * READING "NOTHING" IS SAFE; WRITING OVER IT IS NOT. `readOwned` degrades to an empty list for four
 * different reasons -- the file is missing, its JSON does not parse, it is not an array, or every
 * entry in it was unrecognisable -- and only the first of those means there is nothing there. The
 * caller cannot tell them apart, reads "no processes", and then WRITES: `recordStarted` and
 * `recordStopped` both read-then-write, so an ordinary spawn is enough, and `clearOwned` replaces the
 * file outright. Whatever the damaged file named is then gone, and the processes it named are
 * unreapable for ever with nothing left to say they existed.
 *
 * This is the one place all four writes pass through, which is why the guard lives here instead of at
 * each call site: a fifth writer added later inherits it.
 *
 * IT DOES NOT REFUSE THE WRITE, and that is deliberate. This file's contract is that damaged
 * bookkeeping must never become an outage -- refusing would leave a live process untracked, which is
 * the leak the record exists to prevent. Preserving the old file costs one rename and loses nothing.
 */
function preserveUnreadable(file) {
  ...
  try {
    ...
    // Missing is the ordinary first-run state. There is nothing to preserve.
    return;
  }
  if (raw.trim() === "") return;
  ...
  try {
    ...
    // An array whose entries we ALL rejected is not an understood file either: something wrote
    // records in a shape this version cannot read, and overwriting is the same loss.
    understood = Array.isArray(parsed) && (parsed.length === 0 || parsed.some(isProcessRecord));
  } catch {
    understood = false;
  }
  ...
  try {
    ...
    process.stderr.write(
      `[aify-env] ${file} could not be read as a process record, so it was NOT overwritten. `
      + `Kept at ${aside}. Any process it named is now untracked and must be ended by hand.\n`,
    );
  } catch {
    // Best effort, like the write itself. If the rename fails the write below still proceeds: an
    // environment that cannot record what it starts is worse than a lost damaged file.
  }
}
