...
// the DEFAULTS. A flag written as a word (`"false"`, `"off"`) is not the wrong shape: it is read with
// the words the environment variable accepts. A host preference that refuses to load must never stop
// a daemon from starting or an operator from attaching -- the worst it may do is leave you on the
// default transport.
//
...
/** A flag as written: a boolean, or one of the words above. Null when it says nothing usable. */
...
export function flagFromEnv(env, name) {
  return flagFrom(String(env?.[name] ?? ""));
}
...
export function hostConfigFrom(text, env = {}) {
  ...
  for (const [key, spec] of Object.entries(HOST_SETTINGS)) {
    ...
    // THE SAME WORDS AS THE ENVIRONMENT. Only a boolean was read here, so a hand-written `"false"`
    // fell through to the default -- which is ON, the opposite of what the operator wrote.
    const fromFile = flagFrom(transport[key]);
