 ...
 */
export const DAEMON_FLAGS = ["--version", "--force", "--port", "--instance-context"];
...
export const USAGE = [
  ...
  "  aify-env herdr [agent]      open an existing worker in Herdr; no argument shows a picker",
  ...
  "  --instance-context <path>   herdr-aify env's instance; no takeover, still connects to services",
  ...
  "  p                           show or hide the console pane beside the list",
  "  m                           actions for the selected agent (attach, stop)",
  ...
  "  Ctrl+L                      redraw the screen, if anything has smeared it",
  "  q                           leave the view (aify-env tui only)",
  "  Ctrl+C                      in a menu, the start list or find: back out. On the list:",
  "                              aify-env tui leaves; the daemon's own terminal asks, and a",
  "                              single y stops the environment and every worker it owns",
  "",
...
export function unknownFlag(args, known = DAEMON_FLAGS) {
  ...
  for (let at = 0; at < list.length; at += 1) {
    ...
    if (arg === "--port" || arg === "--instance-context") at += 1;
  }
  return null;
}
