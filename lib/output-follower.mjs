...
export class OutputFollower {
  constructor({
    endpoint, id, buffer = null, fetchImpl = fetch, maxLines = undefined,
    //: HOW A SCREEN IS MADE, injected for the same reason `fetchImpl` is: the failure paths
    //: are the ones worth testing and a real emulator will not fail on demand. `create()`
    //: resolves null for an absent package, but the constructor and the Unicode addon can
    //: REJECT after the module has loaded, and that path was left holding the backlog when
    //: the other two were fixed.
    createScreen = (geometry) => ScreenEmulator.create(geometry),
  } = {}) {
  ...
  #openScreen(meta) {
    ...
    this.createScreen({ cols: meta.cols, rows: meta.rows })
      .then((screen) => {
        this.screenOpening = false;
        // NO EMULATOR ON THIS HOST. `create()` resolves NULL rather than rejecting when the optional
        // package is absent, so this is the ordinary degraded path and not an error.
        //
        // NOTHING IS REPORTED HERE, DELIBERATELY, and it is worth saying so beside the two branches
        // that do report: the pane draws from the line buffer both before and after this moment, so
        // nothing about what it can show has changed. A notification would schedule a repaint that
        // could only draw the same frame.
        //
        // THE HELD BYTES ARE RELEASED, THOUGH. `#paint` holds arriving text while the import is in
        // flight so it can be replayed into the screen; with no screen coming, nothing will ever read
        // them and only the success branch empties the queue. On a stream that was busy during the
        // import that is a buffer retained for the life of the follower, for no reader -- the same
        // shape as the width cache holding its keys' parents.
        //
        // STOPPED WHILE WE WERE LOADING is the other way here, and the ordinary case rather than an
        // edge one: an operator can hide a pane inside the same second they opened it. Disposing
        // rather than storing is what stops a screen outliving the pane that asked for it.
        //
        // ONE BRANCH FOR BOTH, because they are one condition with two causes -- no screen will ever
        // read the backlog. Two returns meant two places to remember to release it, and one of them
        // did not.
        if (!screen || this.stopped) {
          screen?.dispose();
          this.pending = [];
          return;
        }
        ...
        // THE SCREEN ITSELF ARRIVING IS A CHANGE, and the backlog does not always carry one. Each
        // replayed event reports when its parse completes, so a NON-empty backlog is already covered
        // -- but an emulator that loads with nothing waiting would otherwise install silently and
        // the view would keep drawing the pre-emulator frame until something else happened.
        this.#reportProgress();
      })
      .catch(() => {
        this.screenOpening = false;
        // THE THIRD WAY NO SCREEN ARRIVES, and it was left holding the backlog when the other two
        // were fixed. `create()` resolves NULL for an absent package, but the constructor and the
        // addon can REJECT after the module has loaded -- so this is a distinct path, and "no screen
        // will ever read the backlog" is true of it too. (External review of `7b3ba49`.)
        this.pending = [];
        // AND FAILING TO LOAD ONE IS REPORTED. The pane falls back to the line buffer, which is a
        // different screen from the one the view drew while the import was in flight.
        this.#reportProgress();
      });
  }
  ...
  #applyWrite(text, repaints) {
    this.applying = this.applying
      ...
      .catch(() => {})
      // AND SO DOES THE NOTIFICATION, for exactly the same reason. Reporting when the bytes ARRIVED
      // told the view to draw a screen the parser had not touched yet: it drew the refusal, and
      // nothing reported again once the parse landed, so the refusal stood and input stayed blocked
      // until the next collection. Measured by review: display writes stayed 3 -> 3 across several
      // repaint intervals, and only a resize produced the fourth frame.
      //
      // AFTER THE CATCH, so a parse that FAILED still reports. What the pane shows changes either
      // way, and a view that is never told about a failure keeps drawing the frame before it.
  ...
  #applyResize(cols, rows) {
    this.applying = this.applying
      ...
      .catch(() => {})
      // A RESIZE REDRAWS EVERY ROW, so it is a pane change like any other.
      .then(() => this.#reportProgress());
  }
  ...
  paneProblem() {
    ...
    // ASKED OF THE BUFFER, NOT DERIVED A SECOND TIME. This used to return the baseline verdict
    // alone, which is ONE of the three reasons the pane shows a notice -- so a painting process
    // with no emulator, and a stream that had concealed, both rendered a refusal while this
    // reported nothing wrong and `inputIsLive()` let keystrokes through. Review measured the
    // first: notice on screen, problem "", input forwarded.
    const unsound = baselineIsSound(this.meta, this.repaintedSince)
      ? ""
      : baselineProblem(this.meta, this.repaintedSince);
    // THE SAME NULLNESS `lines()` BUILDS, because the buffer distinguishes "no screen at all"
    // from "a screen with a problem" and the rows are not part of that question.
    const screen = this.screen || unsound ? { rows: [], problem: unsound } : null;
    return this.buffer.refusalReason(screen);
  }
  ...
  #consume(text) {
    ...
    for (const frame of frames) {
      ...
      if (result.meta) {
        ...
        // ONLY THE FIRST META CARRIES A VERDICT. `truncated` and `resized` describe the replay this
        // subscriber was handed when it joined; a later meta is the daemon re-announcing geometry,
        // and its `resized` is true precisely because of the resize this follower just applied in
        // order. Taking that verdict froze every watching pane on the first resize, with no way out.
        this.meta = this.meta
          ? { ...this.meta, cols: result.meta.cols, rows: result.meta.rows }
          : result.meta;
  ...
  /**
   * Move to a new status, and tell the owner when it actually changed.
   *
   * ONE SETTER RATHER THAN A CALL PER BRANCH. There are seven places this status is assigned --
   * connecting, gone, two flavours of failed at the response, streaming, a stream that ended with
   * no exit, exited, and `#fail` -- and review found the notification missing from every one of
   * them: a 404 left the follower GONE while the last frame still said "connecting...", with the
   * write count stuck at 2. Adding six calls would have worked until somebody added an eighth
   * branch, which is the same defect with a delay on it.
   *
   * ONLY ON A CHANGE, so a status re-asserted inside a loop does not schedule repaints for ever.
   */
  #setStatus(next) {
  ...
  #fail(error) {
    ...
    // look like a broken connection. ONLY THAT ONE (v0.7.1 review, W13): the fetch layer raises the
    // same error for a connection it abandoned, and a pane reconnects only from FAILED.
    if (this.stopped && error?.name === "AbortError") return this;
    this.#setStatus(FAILED);
  ...
  lines({ height = 10, width = 80, color = false, cursor = false } = {}) {
    ...
    const screen = this.screen || unsound
      ? { rows: this.screen ? this.screen.rows({ color, cursor }) : [], problem: unsound }
      : null;
