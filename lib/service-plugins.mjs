...
export class PluginProcesses {
  ...
  /**
   * @param onStarted  optional, called with the record after a successful start. A CONVENIENCE that
   *   must never become a dependency: it is awaited so ordering is predictable, and a throw from it
   *   is swallowed, because what it does (opening a Herdr pane onto the worker) is decoration on a
   *   process that is already running.
   * @param prepare    optional, turns the spec into the one actually started (the pane opener names
   *   the file the worker's launcher learns its Herdr pane from). A throw starts the spec as given.
   */
  constructor(runner, { onStarted = null, prepare = null } = {}) {
  ...
  async start(spec) {
    let prepared = spec;
    if (this.#prepare) {
      try { prepared = this.#prepare(spec); } catch { prepared = spec; }
    }
    const record = await this.#runner.start(prepared);
    if (this.#onStarted) {
      // THE SPEC'S LABEL TRAVELS WITH THE RECORD. A started record carries the process id and not
      // always the agent's name, and a space called `aify-env 68583473-ccaa-...-p1` tells an operator
      // looking for `sc-tester` nothing. The record wins where it has one.
      const named = record?.label ? record : { ...record, label: spec?.label ?? record?.label };
      try { await this.#onStarted(named, prepared); } catch { /* a missing pane never fails a start */ }
    }
    return record;
  }
  ...
  /** What this process's screen shows, as rows of text with its title; null when there is no screen. */
  screenText(id) { return this.#runner.screenText(id); }
...
export class ServicePlugins {
  ...
  /**
   * What a STARTED plugin offers this host beyond its own lifecycle, by name.
   *
   * A SEPARATE NAMESPACE, AND THAT IS A REQUIREMENT RATHER THAN TIDINESS. A plugin already owns
   * `name`, `state`, `start` and `stop` -- so a capability called `start` reached as a bare property
   * would be the plugin's own lifecycle start, and a route wired to it would boot the plugin again
   * instead of starting an agent. Capabilities live under `capabilities` where they cannot collide
   * with the contract they sit beside.
   *
   * STARTED, not registered, for the same reason `report()` is: a plugin that failed to start has no
   * working api behind it, and handing its capability out would produce a route that answers with a
   * connection error instead of saying the plugin is not running.
   *
   * @returns {object|null} the capability, or null when no started plugin offers one
   */
  capability(name) {
    const wanted = String(name || "");
    if (!wanted) return null;
    for (const plugin of this.#started) {
      const offered = plugin?.capabilities?.[wanted];
  ...
  /**
   * What each STARTED plugin says about itself, for `/health` and the doctor.
   *
   * WHY THIS EXISTS. A plugin's whole job can fail while the daemon runs perfectly: on 2026-09-02
   * the aify-comms plugin beat every 30 seconds for hours and was discarded every time, and nothing
   * anywhere could say so -- `/health` reported the ADVERTISER (which was fine) and knew nothing of
   * the claimer (which was not). An operator asking "is aify-env working" got yes.
   *
   * STARTED, not registered: a plugin that failed to start has no state to report and saying
   * nothing about it would read as healthy. Those are named by `bootstrapReport` at boot instead.
   *
   * A plugin need not implement `state()`. One that does not reports `{}` rather than being omitted,
   * because a missing NAME reads as "no such plugin" and a missing STATE reads as "nothing to say".
   */
  report() {
    return this.#started.map((plugin) => ({
      name: plugin.name,
