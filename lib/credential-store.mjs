 ...
 *
 * The digest is over the exact service identity, so a collision no longer follows from two names
 * being mangled or truncated into the same prefix. The prefix is kept only so an operator listing
 * the store can still tell which file belongs to what.
 *
 * IT IS COLLISION-RESISTANT, NOT COLLISION-FREE, and the earlier wording here claimed the second.
 * Twelve hex characters is 48 bits, so this is a bounded improbability rather than a guarantee, and
 * certifying mathematical uniqueness from a truncated hash is exactly the kind of claim that gets
 * believed and never rechecked. What actually PREVENTS a collision is the registry closure refusing
 * two entries whose references case-fold together, plus the store refusing a colliding write; this
 * derivation only makes an accidental one vanishingly unlikely.
 *
...
/**
 * Which stored credentials nothing references, and which references have no file.
 *
 * TWO DIFFERENT PROBLEMS WITH TWO DIFFERENT OWNERS. An ORPHAN is a secret sitting on disk that
 * nothing will ever present -- left by a service that was removed, or by a rotation that changed the
 * reference. It is not urgent and it is not this daemon's to delete: a file nobody references today
 * may be referenced by a registry that is currently unreadable, and auto-deleting a population on
 * that reasoning is how a cleanup becomes an outage. A DANGLING reference is the opposite: something
 * IS pointed at a file that is not there, so a service will fail to advertise until it is fixed.
 *
 * MATCHED CASE-FOLDED, because on a case-insensitive volume `Foo.key` and `foo.key` are one file --
 * so a reference spelled differently from the file on disk is not an orphan, it is the same secret
 * under another name, and reporting it as unreferenced would invite somebody to delete a live key.
 *
 * PURE: both lists are handed in.
 */
export function credentialOrphans({ storeNames = [], registryRefs = [] } = {}) {
  const files = (Array.isArray(storeNames) ? storeNames : []).filter((n) => typeof n === "string");
  const refs = (Array.isArray(registryRefs) ? registryRefs : [])
    .filter((r) => typeof r === "string" && r.trim() !== "");

  const foldedRefs = new Set(refs.map((r) => r.trim().toLowerCase()));
  const foldedFiles = new Set(files.map((f) => f.toLowerCase()));

  return {
    orphans: files.filter((f) => !foldedRefs.has(f.toLowerCase())).sort(),
    dangling: refs.filter((r) => !foldedFiles.has(r.trim().toLowerCase())).sort(),
  };
}
