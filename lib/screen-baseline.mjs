...
// than a more generous rule here. That checkpoint now exists (`screen-checkpoint.mjs`) and a meta
// declaring one is sound; this rule remains for the replay, which is what a host without the
// optional packages, or a process that has exited, still sends.
//
 ...
 *
 * AND A THIRD WAY TO BE UNSOUND, ADDED 2026-09-08: THE GEOMETRY MOVED UNDER THE BYTES.
 *
 * A late subscriber is handed the retained text plus the CURRENT size, and if the PTY was resized
 * after those bytes were written, the two never described the same moment. Measured against the real
 * parser: 60 characters and an `O` written at 80 columns, then a resize to 40. The real terminal
 * reflows and shows one row; replaying the same bytes into a 40-column emulator puts the `O` on row
 * two. Both screens are coherent. Only one of them is what the process is looking at.
 *
 * IT IS THE SAME CLAIM AS TRUNCATION AND TAKES THE SAME REMEDY -- "these bytes cannot rebuild the
 * positioned screen", cleared by a full repaint and by nothing weaker. So it is folded into the same
 * answer rather than given a second gate that a caller could consult one of.
 *
 * @param {{truncated?: boolean, resized?: boolean, checkpoint?: boolean}|null} meta what the producer declared, or null
...
export function baselineIsSound(meta, repaintedSince) {
  ...
  // A CHECKPOINT IS SOUND BY CONSTRUCTION: the daemon's own emulator saw every byte and every resize
  // in order and serialized the screen at a known position, followed by exactly the bytes after it.
  // `truncated` and `resized` still describe the retained replay honestly, and no replay was sent.
  if (meta && meta.checkpoint === true) return true;
  // A DECLARED RESIZE IS UNSOUND WHATEVER `truncated` SAYS. A complete history whose geometry moved
  // is exactly the case review found: nothing was lost, the daemon's `truncated: false` was honest,
  // and the reconstruction was still wrong. Checked BEFORE completeness so the two cannot be read as
  // alternatives -- soundness needs both, and a stream that satisfies one is not sound.
  if (meta && meta.resized === true) return false;
...
export function baselineProblem(meta, repaintedSince) {
  ...
  // NAMED SEPARATELY, because the two have different causes and an operator reading "earlier output
  // was dropped" for a resize would go looking for a buffer problem that is not there.
  if (meta.resized === true) {
    return "waiting for the first full repaint — the terminal was resized, so these bytes were written at another width";
  }
