 ...
 */
export function paneTitle(pane, width, height = 0) {
  ...
  //
  // AND WHETHER THOSE KEYS ARE ARRIVING (v0.7 scan, F6). Attached to a pane that shows a refusal or a
  // stream that is not live, every key is dropped; "typing here" over that was the pane lying.
  const paused = String(pane?.inputPaused ?? "");
  const attached = pane?.attached !== true ? ""
    : paused ? ` · input paused: ${paused}` : " · typing here";
  ...
  const cropped = producerCols > 0 && width > 0 && producerCols > width
    ? [` · ${producerCols} cols, cropped`, ` · ${producerCols}c cropped`]
    : null;
  ...
  const unicode = pane?.screenUnicode && pane.screenUnicode !== "11"
    ? [` · unicode ${pane.screenUnicode}, columns may differ`, ` · unicode ${pane.screenUnicode}`]
    : null;
  // A QUALIFICATION CUT IN HALF IS WORSE THAN ONE THAT IS ABSENT, and that is what this header did.
  //
  // MEASURED by review at the supported width: a 132-column agent at Unicode 6 in an 80-column
  // terminal gives the pane 40 columns, and the title read `... cropped · unic`. The version number
  // and the sentence explaining what it means were both gone -- so the one line telling an operator
  // the columns may be wrong said nothing at all, while looking like it had said something.
  //
  // THE LABEL ABSORBS THE LOSS, NOT THE WARNING. An agent's name is identifiable from three or four
  // characters beside a row the operator just selected; "the screen may be measured wrongly" is not
  // recoverable from a fragment. So the qualifications are fitted first and the head is clipped to
  // whatever is left.
  // COMPUTED ONCE. Calling it twice -- for the reservation and for the text -- would be two answers
  // to one question, and the day they disagree is the day the header is cut in a way no test drove.
  // AND WHETHER ROWS ARE HIDDEN (v0.7 scan, F5). The pane shows the bottom of a taller screen -- the
  // live edge -- so the rows above it are the ones out of view, and the title says so rather than
  // letting a missing transcript read as one the agent never printed. Silent when the caller gave no
  // height: that caller has claimed nothing about rows.
  //
  // COUNTED FROM WHAT WAS PAINTED, not from the PTY's height (v0.7.1 review, E4): the pane keeps the
  // last painted rows, so a 50-row PTY with 10 painted rows hides nothing in a 20-row pane.
  const above = Math.max(0, (Number(pane?.paintedRows) || 0) - (height > 0 ? height : Infinity));
  const rowsCropped = above > 0
    ? [` · ${above} rows above, cropped`, ` · ${above}r above`]
    : null;
  const tail = qualified(width, [unicode, cropped, rowsCropped]);
  return clipToWidth(`${mark} ${label}${exit}${attached}`, Math.max(0, width - tail.length)) + tail;
}

//: The least a title may show of the agent's own name before the qualifications start being dropped
//: instead. Below this the header stops identifying anything and the trade turns the other way.
const MIN_LABEL_ROOM = 12;

/**
 * As much of the qualification tail as fits WHOLE, in priority order.
 *
 * `candidates` is most-important first, each `[full, short]`. Each is taken at its full form if
 * there is room, then at its short form, and dropped entirely before it would be cut -- and the
 * lowest-priority one is dropped first, so the warning that says the screen may be WRONG survives
 * the note that says it is merely PARTIAL.
 */
function qualified(width, candidates) {
  const present = candidates.filter(Boolean);
  if (!present.length || !(width > 0)) return present.map(([full]) => full).join("");
  const room = Math.max(0, width - MIN_LABEL_ROOM);
  // Longest acceptable rendering first: every candidate full, then degrade the LAST one at a time to
  // its short form, then drop from the end. Priority order is the array's order.
  for (let dropped = 0; dropped <= present.length; dropped += 1) {
    const kept = present.slice(0, present.length - dropped);
    for (const shortFrom of [kept.length, ...kept.map((_, i) => kept.length - 1 - i)]) {
      const text = kept.map(([full, brief], i) => (i >= shortFrom ? brief : full)).join("");
...
export function composeConsole({
  ...
  const body = typeof pane.lines === "function"
    // THE CARET ONLY WHEN THE KEYBOARD IS IN THIS PANE (v0.7 scan, F11): that is when an operator is
    // typing at the agent's input line and needs to see where it is.
    ? pane.lines({ height: bodyHeight, width: paneWidth, color, cursor: pane.attached === true })
    : [];

  const right = [
    paneTitle(pane, paneWidth, bodyHeight),
